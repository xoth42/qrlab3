...
class CavSpectroscopy(Measurement1D):
    ...
    def __init__(self, cav_source, qubit_info, cav_info, cav_disps, cav_freqs, plot_type=None):
        ...
        super(CavSpectroscopy, self).__init__(1)
    ...
    def generate(self, amp):
        ...
        s = Sequencer(s)
        seqs = s.render()
        if self.qubit_info and self.qubit_info.ssb:
            self.qubit_info.ssb.modulate(seqs)
        if self.cav_info and self.cav_info.ssb:
