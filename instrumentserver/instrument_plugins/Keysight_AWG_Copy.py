...
class Keysight_AWG_Copy(Instrument):


    def __init__(self, name, chassis=1, slot=7, AWG_PRODUCT = "M3202A"):
        super(Keysight_AWG_Copy, self).__init__(name)
#        self.set_timeout(120000)
        ...
        self.waveform_dict = {}
        self.waveform_list = []
        self.channel_amps = np.zeros(4)
        self.channel_ofs = np.zeros(4)
        self.waveform_queue = 0
        self.n_el = 0
        self.m1 = []
        self.m2 = []
        self.waveform_delay = 250000
...
#            flags=Instrument.FLAG_GET)
        ...
        for i in range(4):
            self.do_set_amplitude(1, i+1)
            self.do_set_offset(0, i+1)
...
#        self.awg.AWGqueueWaveform(2, 0, key.SD_TriggerModes.SWHVITRIG, 0, 1, 0)
    ...
    def do_get_serial(self):
        return self.awg.getSerialNumberBySlot(self.chassis, self.slot)

    def do_get_part(self):
        return self.awg.getProductNameBySlot(self.chassis, self.slot)
    ...
    def do_get_waveform_delay(self):
        return self.waveform_delay
        
    def do_set_waveform_delay(self, delay):
    ...
    def prime(self):
        ...
        for n in range(4):
            ...
            self.awg.AWGqueueWaveform(n+1, n, key.SD_TriggerModes.SWHVITRIG, 0, 1, 0)
...
#        print('wname', wname, 'm1', m1, 'm2', m2, 'waveform_num', self.waveform_num)
        
        self.waveform_list += [data]
        self.waveform_dict[wname] = self.waveform_num
        self.m1 += [m1]
        self.m2 += [m2]
        self.waveform_num += 1
...
#        print('ch', ch, 'el', el, 'wname', wname, 'repeat', repeat, 'trig', trig)
        self.waveform_queue[ch-1, el-1] = self.waveform_dict[wname]
