# -*- coding: utf-8 -*-
"""
Created on Thu Dec 30 12:08:26 2021
...
class Agilent_E5080B(SCPI_Instrument):
    '''
    This is the driver for the Agilent E5080B Vector Network Analyzer
    ...
    def __init__(self, name, address, **kwargs):
        ...
        super(Agilent_E5080B, self).__init__(name, address)
        ...
        self.add_scpi_parameter('format', 'CALC:MEAS:FORM', '%s',
        ...
        self.add_scpi_parameter('trigger_source', 'TRIG:SOUR', '%s',
                           ...
                           format_map={"INT": "internal",
                                       ...
                                       "MAN": "manual",
                                       "BUS": "external bus"})
        self.add_scpi_parameter('sweep_mode', 'SENS:SWE:MODE', '%s',
                           type=types.StringType, flags=Instrument.FLAG_GETSET,
                           format_map={"HOLD": "HOLD",
                                       "CONT": "CONTinuous",
                                       "GRO": "GROups",
                                       "SING": "SINGle"})
...
#                                       "0": "disallowed"}) # requires firmware upgrade
        self.add_scpi_parameter('clock_reference', 'SENS:ROSC:SOUR', '%s',
                           ...
                           format_map={"INT": "internal",
                                       "EXT": "external"})
        self.add_scpi_parameter('electrical_delay', 'CALCulate:MEASure:CORRection:EDELay',
                                '%s', units="s", type=types.FloatType)
...
#        return np.array(map(float, self.ask('CALC:DATA:XAXis?', timeout=1).split(',')))
        return np.linspace(self.get_start_freq(), self.get_stop_freq(),self.get_points())

    def do_get_yaxes(self):
        strdata = self.ask('CALCulate1:MEASure1:DATA:FDATA?', timeout=0.1)
        data = np.array(map(float, strdata.split(',')))
#        data = data.reshape((len(data)/2, 2))
        return data[:,None].T # mags, phases
...
#Yingying add opc_w and opc_r        
    ...
    def set_s_param(self, sij):
        self.write(':CALCulate1:MEASure1:PARameter %s' % (sij))
        
    def start_coll_data(self, ave):
        self.write('SENS:SWE:GRO:COUN %s'%(ave))
        self.set_sweep_mode('GRO')
        
    def stop_coll_data(self):
        self.set_sweep_mode('CONT')
if __name__ == '__main__':
    logging.getLogger().setLevel(logging.DEBUG)
    Instrument.test(Agilent_E5080B)
    # vna.do_get_data()
