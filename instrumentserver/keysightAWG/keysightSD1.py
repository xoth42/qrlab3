...
class SD_Module(SD_Object) :
	...
	def scan(cls):
		...
		1. Calls SD_Module.moduleCount() to determine how many SD1 modules are present.
		   If the result is negative, an empty list is returned and the error code
		...
		2. For each module index in range(nModules):
		   - Uses the static/class methods:
		     ...
		     * SD_Module.getTypeByIndex(index)
		     ...
		     Any SD1 error codes (negative integers) or Unicode decode errors are
		     handled gracefully:
		     * The raw error code is stored in the corresponding field.
		     * A best-effort string or None is stored for the string fields.
		...
		- "type_enum"   : int or int-like error
		  ...
		  If a negative SD1 error code is returned by getTypeByIndex(), that code
		...
		- "product"     : str or None or int
		  Product name / part number for the module, e.g. "M3202A".
		  If getProductNameByIndex() returns a negative SD1 error code or raises a
		  UnicodeDecodeError, the raw error code is stored here or the value is
		  set to None; details are logged at DEBUG level.

		- "serial"      : str or None or int
		  Module serial number string as reported by SD_Module.getSerialNumberByIndex().
		  Same error-handling conventions as for "product".

		- "chassis"     : int
		  Chassis number in which the module resides, as reported by
		  SD_Module.getChassisByIndex(). If a negative SD1 error code is returned,
		  that code is stored here.

		- "slot"        : int
		  Slot number within the chassis, as reported by SD_Module.getSlotByIndex().
		  If a negative SD1 error code is returned, that code is stored here.

		- "open_status" : int
		  Return value from openWithSlot(product, chassis, slot). A positive value
		  indicates a valid handle. A negative value is an SD_Error code. If the
		  scan determined that opening was not attempted (e.g. invalid product /
		  chassis / slot), this field is set to that reason code or
		  SD_Error.INVALID_SLOT.
		...
		- "status"      : int or None
		  ...
		  code). Negative values are SD_Error codes. If getStatus() itself fails or
		  is not called, this may be None.
		...
		- "self_test"   : int or None
		  Result of runSelfTest() if the module was opened successfully. Zero
		  indicates success; negative values are SD_Error codes. If the module
		  could not be opened or self-test was not run, this is None.

		Errors and diagnostics
		----------------------
		- If SD_Module.moduleCount() returns a negative value, an empty list is
		  returned and the error is logged with its SD_Error message.

		- For individual modules, any negative "open_status", "status",
		  "type_enum", "chassis", "slot", or "self_test" values are error codes
		  from the SD1 API (see SD_Error). You can map these to text using
		  SD_Error.getErrorMessage(code) in your own code if desired.

		- If any of the get*ByIndex methods return invalid string data (e.g. causing
		  UnicodeDecodeError), this is logged at DEBUG level and the offending field
		  is set to None; the rest of the scan continues.

		Side effects
		------------
		- Temporarily opens and then closes each discoverable module once during the
		  scan (when possible). It does not modify any configuration or start/stop
		  any AWG/DAQ engines.

		- Does not perform any HVI operations or synchronization; it is purely a
		  discovery / status / self-test helper.
		...
		n = cls.moduleCount()
		if n < 0:
			# Global driver-level error; log and return empty list.
			try:
				msg = SD_Error.getErrorMessage(n)
			except Exception:
				msg = "<could not decode SD1 error>"
			logger.debug("SD_Module.moduleCount() failed with error %d: %s", n, msg)
			return []

		logger.debug("SD_Module.scan(): moduleCount -> %d", n)
		...
		for idx in range(n):
			...
			# --- Product name ---
			try:
				tmp = cls.getProductNameByIndex(idx)
				if isinstance(tmp, int) and tmp < 0:
					# SD1 error code
					logger.debug(
						"getProductNameByIndex(%d) error %d: %s",
						idx, tmp, SD_Error.getErrorMessage(tmp)
					)
					prod = tmp  # store error code
				else:
					prod = tmp
			except UnicodeDecodeError as e:
				logger.debug(
					"getProductNameByIndex(%d) raised UnicodeDecodeError: %r",
					idx, e
				)
				prod = None
			...
			# --- Serial number ---
			try:
				tmp = cls.getSerialNumberByIndex(idx)
				if isinstance(tmp, int) and tmp < 0:
					logger.debug(
						"getSerialNumberByIndex(%d) error %d: %s",
						idx, tmp, SD_Error.getErrorMessage(tmp)
					)
					sn = tmp
				else:
				...
				logger.debug(
					"getSerialNumberByIndex(%d) raised UnicodeDecodeError: %r",
					idx, e
				)
				sn = None
			...
			if ch < 0:
				logger.debug(
					"getChassisByIndex(%d) error %d: %s",
					idx, ch, SD_Error.getErrorMessage(ch)
				)

			slot = cls.getSlotByIndex(idx)
			if slot < 0:
				logger.debug(
					"getSlotByIndex(%d) error %d: %s",
					idx, slot, SD_Error.getErrorMessage(slot)
				)

			mtype = cls.getTypeByIndex(idx)
			if mtype < 0:
				logger.debug(
					"getTypeByIndex(%d) error %d: %s",
					idx, mtype, SD_Error.getErrorMessage(mtype)
				)

			type_name = TYPE_NAMES.get(mtype, f"Unknown ({mtype})")

			# Decide whether we can even attempt to open this module
			can_open = (
				isinstance(prod, str) and
				isinstance(ch, int) and ch >= 0 and
			...
			open_status = SD_Error.INVALID_SLOT if not can_open else 0
			status = None
			fw_ver = None
			hw_ver = None
			self_test = None
			...
			if not can_open:
				logger.debug(
					"scan(): skipping open for index %d (product=%r, chassis=%r, slot=%r)",
					idx, prod, ch, slot
				)
				# open_status already set to INVALID_SLOT in this case
			else:
				open_status = mod.openWithSlot(prod, ch, slot)
				logger.debug(
					"scan(): openWithSlot(%r, %r, %r) for index %d -> %d",
					prod, ch, slot, idx, open_status
				)

				if open_status <= 0:
					# Could not open; record as status and leave others None
					status = open_status
					try:
						msg = SD_Error.getErrorMessage(open_status)
					except Exception:
						msg = "<could not decode SD1 error>"
					logger.debug(
						"scan(): open failed for index %d with %d: %s",
						idx, open_status, msg
					)
				else:
					# Opened successfully; gather status and versions
					status = mod.getStatus()
					fw_ver = mod.getFirmwareVersion()
					hw_ver = mod.getHardwareVersion()
					self_test = mod.runSelfTest()

					logger.debug(
						"scan(): index %d status=%r fw=%r hw=%r self_test=%r",
