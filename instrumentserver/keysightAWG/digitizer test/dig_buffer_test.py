...
def digitizer_setup(dig, nsamples, captures, ntransfers, captureDelay = 0, digScale = 2):
    digChannels = [1, 2] 
    ...
    for i in range(len(digChannels)):   
       ...
       dig.DAQconfig(digChannels[i], nsamples, captures, captureDelay, key.SD_TriggerModes.EXTTRIG)
       dig.DAQbufferPoolConfig(digChannels[i], nsamples * captures / ntransfers)

def digitizer_acquire(dig, hvi, awg, nsamples, npoints, ntransfers, data_channel):
    captures = npoints * naverages
    digChannels = [1, 2]
...
#    return data
    points_per_transfer = nsamples * captures / ntransfers
    temp = np.zeros(nsamples*npoints, dtype = np.float64)
    for transfer in range(ntransfers):
        try:
            if transfer % (ntransfers/100) == 0: 
                print(transfer, ' averages done')
                gc.collect()
        except:
...
#        stds[transfer * points_per_transfer : (transfer+1) * points_per_transfer] = temp**2
        ...
        for i in range(npoints):
            sums[i] += temp[i * nsamples : (i+1) * nsamples]
            sums_squared[i] += temp[i * nsamples : (i+1) * nsamples]**2
...
naverages = 1000000 # total number of averages per point
captures = npoints * naverages # total number of acquistions for dig to take
ntransfers = naverages # number of blocks it takes the dig data to transfer to the pc
nchannels = 4
data_channel = 1
...
digitizer_setup(dig, nsamples, captures, ntransfers)
start_time = time.time()
print('starting acquisition')
means, stds = digitizer_acquire(dig, hvi, awg, nsamples, npoints, ntransfers, data_channel)
