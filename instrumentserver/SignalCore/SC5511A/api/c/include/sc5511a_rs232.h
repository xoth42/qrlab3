/*
 ***************************************************************************
 * RS232 functions for SignalCore Inc SC5511A products with the RS232 interface 
 *
 *	Copyright (c) 2013-2018 SignalCore Inc.
 *	
 * Rev 2.0
 *
 ****************************************************************************
 * SC5511A RS232 header file 
*/
...
#define SCI_USB_PID					0x001E
#define SCI_PRODUCT_NAME			"SC5511A"
#define SCI_RS232_INTERFACE_ID		0x02			//USB & RS232
...
//System Registers
#  define SYNTH_SELF_CAL					0x47	// Device performs a self calibration of this internal PLL parameters
...
extern "C"
{
	...
	#define EXPORT_DLL 
#else
	#ifdef FUNCTION_EXPORTS  // Preprocessor
...
//! define the Comm Port Handle type

typedef	HANDLE sc5511a_device_handle_t;

typedef struct   //to handle 2-D in LabView
{
	char **port;
} port_list_t;
...
/* RS232 specific related functions */	
...
*/
EXPORT_DLL int sc5511a_search_devices(char **port_list, int max_devices, unsigned int *count);

EXPORT_DLL int sc5511a_search_devices_lv(char *ports, int max_devices, unsigned int *count);

/*	Function opens the target RS232 device aimed to handle LabVIEW calls.
	return:		pointer to usb_dev_handle type
	input: 		comm_port is the port name string eg "COM1", "COM5"
				the product label.
*/
EXPORT_DLL int	sc5511a_open_device(char *comm_port, unsigned int baud_rate, sc5511a_device_handle_t *dev_handle);

/*	Function  closes RS232 device associated with the handle.
	return:		error code
...
/* 	Writing the register with via the RS232 device handle allocated by SC5511A_OpenDevice
	return: error code
...
/* 	Reading the register with via the RS232 device handle allocated by SC5511A_OpenDevice
	input: reg_byte contains the target register address, eg 0x10 is the frequency register
	input: 64 bit instruct_word contains necessary data for the specified register address
...
/*	Sets the device frequency
	return: error code
	input:	frequency in Hz up to 20,000,000,000 Hz
*/
EXPORT_DLL int sc5511a_set_freq(sc5511a_device_handle_t dev_handle, unsigned long long int freq);

/*	Sets the signal phase
return: error code
input:	phase in degrees 0.1 degrees steps.
*/
EXPORT_DLL int sc5511a_set_signal_phase(sc5511a_device_handle_t dev_handle, float phase);
...
output:	deviceInfo		device information structure
*/
EXPORT_DLL int sc5511a_get_device_signature(sc5511a_device_handle_t dev_handle,
