...
class InstrumentsContainer(QtGui.QTabWidget):
    ...
    def save_settings(self):
        fn = QtGui.QFileDialog.getSaveFileName(directory=config.ins_store_fn)
        if fn:
            self.instruments.save_instruments(fn)

    def load_settings(self, create=False):
        fn = QtGui.QFileDialog.getOpenFileName(directory=config.ins_store_fn)
        if fn:
...
class NoDropOnModel(QtGui.QStandardItemModel):
    def flags(self, idx):
        flags = super(NoDropOnModel, self).flags(idx)
        if idx.isValid():
            flags &= ~QtCore.Qt.ItemIsDropEnabled
        return flags

class ListInstrumentsContainer(QtGui.QSplitter):
    def __init__(self, instruments):
        super(ListInstrumentsContainer, self).__init__()
        self.list_view = QtGui.QListView()
        self.inst_list = NoDropOnModel()
        tabs_container = QtGui.QWidget()
        self.tabs_layout = QtGui.QVBoxLayout(tabs_container)
        self.list_view.setModel(self.inst_list)
        self.addWidget(self.list_view)
        self.addWidget(tabs_container)
        self.list_view.clicked.connect(self.change_current)
        self.list_view.setDragDropMode(self.list_view.InternalMove)
        self.current = None
        self.tabs_layout.addSpacerItem(QtGui.QSpacerItem(500, 0))
    ...
    def _ins_added_cb(self, name):
        ...
        tab_scroll_area.setWidget(tab)
        tab_scroll_area.setWidgetResizable(True)
        tab_scroll_area.hide()
        self.tabs_layout.addWidget(tab_scroll_area)
        self._ins_widgets[name] = tab_scroll_area

        item = QtGui.QStandardItem(name)
        item.setEditable(False)
        self._ins_items[name] = item
        self.inst_list.appendRow(item)
        # self.show_tab_actions[name] = action = QtGui.QAction(name, self)
        # action.setCheckable(True)
        # action.setChecked(True)
        # action.toggled.connect(lambda enabled: self.show_tab(name, enabled))
    ...
    def _ins_removed_cb(self, name):
        ...
        if name in self._ins_widgets:
            ...
            idx = self.inst_list.indexFromItem(item)
            self.inst_list.removeRow(idx.row())
            del self._ins_widgets[name]
...
class InstrumentsMainWindow(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.tab_widget = ListInstrumentsContainer(instruments)
