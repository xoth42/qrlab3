...
class ab_time_domain(Measurement1D):
    ...
    def __init__(self, fwm_info, pump_info, qubit_list, delays, fwm_amp, pump_amp, 
                 fwm_switch, seq=None, postseq=None, bgcor=False,
        ...
        super(ab_time_domain, self).__init__(npoints, infos=[fwm_info, pump_info] + qubit_list, **kwargs)
...
#            dt = 60e3# Chen made some temporarty test changes
            for dt in self.delays:
                s.append(Join([self.seq,
                              Constant(500, 1, chan=self.fwm_switch),
                              ]))
                if dt > 0:
                    s.append(Join([Combined([Constant(int(dt), self.pump_amp, chan=self.pump_info.sideband_channels[0]),
#                                             Constant(int(dt), self.fwm_amp, chan=self.fwm_info.sideband_channels[0]),
