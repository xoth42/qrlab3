...
class T2_AQEC(Measurement1D):

    def __init__(self, qubit_info, delays, comb_list, detune=0, echotype=ECHO_NONE, necho=1,
                 double_freq=False, seq=None, postseq=None, selective=False, **kwargs):
        ...
        infos = [qubit_info] + [comb.info for comb in comb_list]

        super(T2_AQEC, self).__init__(len(delays), infos=infos, **kwargs)
            ...
            if e:
                ...
                for i in range(self.necho):
                    ...
                    for c in self.comb_list:
                        poly_seq += c.get_poly_seq(2*tau - c.sigma*4, 0)
