...
prepareBF = sequencer.Join([sequencer.Trigger(250), gepi, efpi, cB(1.5,0)])
...
if 1: # gf Chi evolution for Cavity A
    from scripts.single_cavity import Qfunction
    for delay in [171, 172, 173, 174, 175]:
        seq = sequencer.Join([prepareAF, sequencer.Delay(delay), cA(1.95, -pi*0.01),
                              efpi, gepi])
...
if 1: # gf Chi evolution for Cavity B
    from scripts.single_cavity import Qfunction
    for delay in [170]:
        seq = sequencer.Join([prepareBF, sequencer.Delay(delay), cB(1.46, pi*0.03),#0.05
                              efpi, gepi])
        Qfun = Qfunction.QFunction(qubit_info, cavity_info1B, amax=2.0, N=9, amaxx=None, Nx=None, amaxy=None, Ny=None,
                     seq=seq, delay=5, bgcor=False, Qswitch_infoA=Qswitch_info1A, Qswitch_infoB=Qswitch_info1B, extra_info=[ef_info, Qswitch_info1A, Qswitch_info1B,])
        Qfun.measure()
