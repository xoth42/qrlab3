...
if 0: # test digitizer DEMODULATED
    dig = mclient.instruments['dig']
    avgs = dig.test_dig_demod(500, 10000)
    print((np.shape(avgs)))
    plt.figure()
...
if 0: #set readout and cavity displacement freq
    ...
    cav_disp = 10.811e9
    power = 10
    mixer_info1_set.set_pi_amp(0.10)
    mixer_info2_set.set_pi_amp(0.10)
    mixer_deltaf = 50e6
    mixer_info1_set.set_deltaf(mixer_deltaf)
    mixer_info2_set.set_deltaf(mixer_deltaf)
    readout_info.rfsource1.set_frequency(ro_freq - mixer_deltaf)
...
#    readout_info.rfsource2.set_frequency(ro_freq+50e6)
    ...
    SS_mixer_info2_set.set_deltaf(deltaf)
    SS_mixer_info1 = mclient.get_qubit_info('SS_mixer_info1')
    SS_mixer_info2 = mclient.get_qubit_info('SS_mixer_info2')
...
#    Yoko.do_set_current(-0.00175)
    rofreq = 10.807e9
    freq_range = 7e6
    ro = rocavspectroscopy_keysight.ROCavSpectroscopy_keysight(qubit2_info, np.linspace(10,-6,1),
...
#        dig.set_nsamples(500)
        ...
        plt.close()
#        
        figure_name =   '0 T 20MHz'
#        figure_name = '-0.019 T qubit_w g/e'
##        
...
#        
        ...
        plt.figure('amp%s'%(figure_name))
        plt.plot(ro.freqs,ro.ampdata[0],label = 'S21, g, amp = %s'%(amps2[i]))
        plt.legend()
        plt.figure('phase%s'%(figure_name))
        plt.plot(ro.freqs,ro.phasedata[0],label = 'g, amp = %s'%(amps2[i]))
        plt.legend()
        
        ro = rocavspectroscopy_keysight_mixer.ROCavSpectroscopy_keysight_mixer(qubit2_info, mixer_info1, mixer_info2, np.linspace(10,10,1),
                                                 np.linspace(rofreq-freq_range, rofreq+freq_range, 101),
                                                 qubit_pulse=True, seq=None, postseq = None,)#readout ='readout_IQ' )#,extra_info=[ef2_info])
        ro.measure()
        plt.close()
        plt.close()
        
        comp_e2 = ro.ampdata[0] * np.exp(1j * ro.phasedata[0]*np.pi/180)
        plt.figure('amp%s'%(figure_name))
        plt.plot(ro.freqs,ro.ampdata[0],label = 'qubit 2 in e')
        plt.plot(ro.freqs, np.abs(comp_e2 - comp_g), label = 'diff g and qubit2 e')
        plt.legend()
        plt.figure('phase%s'%(figure_name))
        plt.plot(ro.freqs,ro.phasedata[0],label = 'qubit 2 in e')
        plt.legend()
...
#    rofreq = 7515.5e6
    amps = np.linspace(0.1,0.1,1)
#    amps2 = np.linspace(0,.1,1)
#    dig.do_set_naverages(30000)
    alz.set_naverages(10000)
    w = 1000
    figure_name = '-0.04T V1'
    for i in range(len(amps)):
#        mixer2_amp = amps[i]
        mixer2_amp = amps[i]
        mixer1_amp = amps[i]
        
        readout_IQ.set_amp(mixer1_amp)
        readout_IQ.set_amp_secondary(mixer2_amp)
        readout_IQ.set_pulse_width(w)
        ...
        repeat = 1
        npoints = 81
        comp_g = np.zeros([repeat, npoints],dtype = complex)
        comp_e = np.zeros([repeat, npoints],dtype = complex)
        comp_e1 = np.zeros([repeat, npoints],dtype = complex)
        for j in range(repeat)[0:]:
            ro = rocavspectroscopy_mixer.ROCavSpectroscopy_Mixer(qubit2_info, mixer_info1, mixer_info2, np.linspace(10,10, 1),
                                                 np.linspace(rofreq - freq_range, rofreq + freq_range,npoints), qubit_pulse=False)
            ro.measure()
            
            comp_g[j] = ro.ampdata[0] * np.exp(1j * ro.phasedata[0]*np.pi/180)
            ...
            ro = rocavspectroscopy_mixer.ROCavSpectroscopy_Mixer(qubit2_info, mixer_info1, mixer_info2, np.linspace(10,10, 1),
                                                 np.linspace(rofreq - freq_range, rofreq + freq_range,npoints), qubit_pulse=True)
            ro.measure()
            
            comp_e[j] = ro.ampdata[0] * np.exp(1j * ro.phasedata[0]*np.pi/180)
            
            plt.figure('amp%s'%(figure_name))
            plt.plot(ro.freqs,ro.ampdata[0],label = ' e qubit 2, amp = %s'%(amps[i]))
            plt.plot(ro.freqs,np.abs(comp_e[j] - comp_g[j]),label = ' ge diff qubit 2, amp = %s'%(amps[i]))
            plt.legend()
            plt.figure('phase%s'%(figure_name))
            plt.plot(ro.freqs,ro.phasedata[0],label = 'e qubit 2, amp = %s'%(amps[i]))
            plt.legend()
            
            ro = rocavspectroscopy_mixer.ROCavSpectroscopy_Mixer(qubit_info, mixer_info1, mixer_info2, np.linspace(10,10, 1),
                                                 np.linspace(rofreq - freq_range, rofreq + freq_range,npoints), qubit_pulse=True)
            ro.measure()
            
            comp_e1[j] = ro.ampdata[0] * np.exp(1j * ro.phasedata[0]*np.pi/180)
            
            plt.figure('amp%s'%(figure_name))
            plt.plot(ro.freqs,ro.ampdata[0],label = ' e qubit 1, amp = %s'%(amps[i]))
            plt.plot(ro.freqs,np.abs(comp_e1[j] - comp_g[j]),label = ' ge diff qubit 1, amp = %s'%(amps[i]))
            plt.legend()
            plt.figure('phase%s'%(figure_name))
            plt.plot(ro.freqs,ro.phasedata[0],label = 'e qubit 1, amp = %s'%(amps[i]))
            plt.legend()
#        g = np.average(comp_g,0)
#        
#        plt.figure('ave_amp%s'%(figure_name))
#        plt.plot(ro.freqs,np.abs(g),label = ' g, amp = %s'%(amps[i]))
#        plt.legend()
#
...
#
#
#        params.add('kappa_prod', value= (np.max(np.abs(g))*0.5e6)**2.001, min = 0)#,vary = False)
#        params.add('omega_c', value=ro.freqs[np.argmax(np.abs(g))]*1.00002,min = ro.freqs[np.argmax(np.abs(g))]*0.9998, max = ro.freqs[np.argmax(np.abs(g))] * 1.0002)#,vary = False)
...
#        result = lmfit.minimize(S21, params, args=(ro.freqs, np.abs(g)))
#        lmfit.report_fit(result.params)
#        print ('total Q: ',result.params['omega_c'].value/result.params['kappa_a'].value)
#
#        fitdata = np.sqrt(result.params['kappa_prod'].value)/(-1j*(ro.freqs-result.params['omega_c'].value)-(result.params['kappa_a'].value)/2.0 )
#        fitdata = fitdata + result.params['roff'].value + 1j*result.params['ioff'].value
#        plt.plot(ro.freqs,np.abs(fitdata), '--',label = 'freq = %s MHz\n kappa = %s MHz'%(result.params['omega_c'].value/1e6,result.params['kappa_a'].value/1e6))
#        plt.legend()                
#        
#        e = np.average(comp_e,0)
#        
#        plt.figure('ave_amp%s'%(figure_name))
#        plt.plot(ro.freqs,np.abs(e),label = ' e, amp = %s'%(amps[i]))
#        plt.legend()
#        params = lmfit.Parameters()
#
#
#
#        params.add('kappa_prod', value= (np.max(np.abs(e))*0.5e6)**2.001, min = 0)#,vary = False)
#        params.add('omega_c', value=ro.freqs[np.argmax(np.abs(g))]*1.00002,min = ro.freqs[np.argmax(np.abs(e))]*0.9998, max = ro.freqs[np.argmax(np.abs(e))] * 1.0002)#,vary = False)
#        params.add('kappa_a', value=1e6, min = 0)#, max = 4e6)#,vary = False)
#        params.add('roff',value = 0)#,vary = False)
#        params.add('ioff',value = 0)#, vary = False)
#                
#    #    datas = realdata[0,:]+ 1j*imagdata[0,:]    
#        result = lmfit.minimize(S21, params, args=(ro.freqs, np.abs(e)))
#        lmfit.report_fit(result.params)
#        print ('total Q: ',result.params['omega_c'].value/result.params['kappa_a'].value)
#
#        fitdata = np.sqrt(result.params['kappa_prod'].value)/(-1j*(ro.freqs-result.params['omega_c'].value)-(result.params['kappa_a'].value)/2.0 )
...
if 0: # alazar cavity spec with cw qubit drive


        
    from scripts.single_cavity import cw_rocavspectroscopy_mixer
#    rofreq = 7515.5e6
    amps = np.linspace(0.4,.5,1)
#    amps2 = np.linspace(0,.1,1)
#    dig.do_set_naverages(30000)
    alz.set_naverages(6000)
    figure_name = 'S31  0T'
    for i in range(len(amps)):
        mixer1_amp = 0.005#amps[i]
        mixer2_amp = amps[i]
        
        mixer_info1_set.set_pi_amp(mixer1_amp)
        mixer_info2_set.set_pi_amp(mixer2_amp)
        mixer_info1_set.set_w(400)
        mixer_info2_set.set_w(400)
#        alz.set_nsamples(4800)
        ...
        comp_e = np.zeros([repeat, npoints],dtype = complex)
        for j in range(repeat)[0:]:
            ro = cw_rocavspectroscopy_mixer.CW_ROCavSpectroscopy_Mixer(qubit2_info, mixer_info1, mixer_info2, np.linspace(10,10, 1),
                                                 np.linspace(rofreq - freq_range, rofreq + freq_range,npoints), qubit_pulse=False)
            ro.measure()
            ...
            plt.figure('amp%s'%(figure_name))
            plt.plot(ro.freqs,ro.ampdata[0],label = ' g+e, amp = %s'%(amps[i]))
            plt.legend()
            plt.figure('phase%s'%(figure_name))
            plt.plot(ro.freqs,ro.phasedata[0],label = 'g+e, amp = %s'%(amps[i]))
            plt.legend()
    ...
    #    datas = realdata[0,:]+ 1j*imagdata[0,:]    
        ...
        plt.figure('ave_amp%s'%(figure_name))
        plt.plot(ro.freqs,np.abs(e),label = ' g+e, amp = %s'%(amps[i]))
        plt.legend()
        params = lmfit.Parameters()
...
if 0: #qubit spectroscopy mixer
    from .single_qubit import spectroscopy_mixer
#    from scripts.single_qubit import spectroscopy_IQ
#    seq = sequencer.Sequence([sequencer.Trigger(250), qubit_info.rotate(np.pi, 0), ef_info.rotate(np.pi, 0)])
#    postseq = sequencer.Sequence(qubit_info.rotate(np.pi, 0))
#    for i in [-20,-15,-10,-5,0,5]:
...
#    for current in np.linspace(-8, -10, 5):
#        yoko.do_set_current(current)
    spec = spectroscopy_mixer.Spectroscopy_Mixer(mclient.instruments['SC_qubit'], qubit_info,mixer_info1,mixer_info2,
                                         np.linspace(qubit_freq-freq_range,
                                                     qubit_freq+freq_range, 101), [10],
                                         plen=50000, amp=0.000000000001, seq=None, postseq=None, plot_seqs=False)
    spec.measure()
    bla

if 0: # qubit spectroscopy alazar mixer
    from .single_qubit import spectroscopy_alazar_mixer
#    from scripts.single_qubit import spectroscopy_IQ
#    seq = sequencer.Sequence([sequencer.Trigger(250), qubit_info.rotate(np.pi, 0), ef_info.rotate(np.pi, 0)])
#    postseq = sequencer.Sequence(qubit_info.rotate(np.pi, 0))
#    for i in [-20,-15,-10,-5,0,5]:
...
#    for current in np.linspace(-8, -10, 5):
#        yoko.do_set_current(current)
    spec = spectroscopy_alazar_mixer.Spectroscopy_Mixer(mclient.instruments['SC_qubit'], qubit_info,mixer_info1,mixer_info2,
                                         np.linspace(qubit_freq-freq_range,
                                                     qubit_freq+freq_range, 101), [10],
                                         plen=5000, amp=0.0000000001, seq=None, postseq=None, plot_seqs=False)
    spec.measure()
    bla    
...
#        seq = sequencer.Join([seq, sequencer.Delay(40)])
        spec = ssbspec.SSBSpec(qubit2_info, np.linspace(-20e6, 20e6, 101), seq=seq, plot_seqs=False, proj_func='phase',
                               readout = 'readout_IQ',)
        spec.measure()
#        plt.close()
    bla
...
#            qubit = mclient.instruments['qubit2ge']
#            qubit.set_pi_amp_selective(pi_amps[j])
#            qubit2_info = mclient.get_qubit_info('qubit2ge')
    #        RObrick.do_set_power(i)
        seq = sequencer.Trigger(600)
...
#        seq = sequencer.Trigger(600)
        ...
        spec = ssbspec_mixer.SSBSpec_mixer(ef2_info, mixer_info1,mixer_info2, np.linspace(-15e6, 15e6, 101), seq=seq,postseq = postseq,
...
#            post_seq = sequencer.Join([qubit_info.rotate(np.pi, 0),sequencer.Delay(70)])
            spec = stark_shift_with_mixer.Stark_shift_with_mixer(qubit_info, mixer_info1,mixer_info2, SS_mixer_info1, SS_mixer_info2,
                                                                 phase1, np.linspace(-20e6, 10e6,101), seq=seq, plot_seqs=True, postseq = None,
                                                                 proj_func='phase')
            spec.measure_keysight()
    #        plt.close()
...
#    pi_amps = np.asarray(pi_amps)
    ...
    for j in range(len(pi_amps)):
        SS_mixer_info2_set.set_pi_amp(pi_amps[j])

        SS_mixer_info = mclient.get_qubit_info('SS_mixer_info2')
#    for j in range(len(delays)):
...
#            seq_in = sequencer.Trigger(600)
#            seq = sequencer.Join([seq_in, SS_mixer_info1.rotate(np.pi, phase1 + phase[i]), sequencer.Delay(delays[j])])
            post_seq = sequencer.Delay(100)
            spec = cw_stark_shift_mixer.CW_Stark_shift_with_mixer(qubit2_info, mixer_info1,mixer_info2, SS_mixer_info,
                                                                 phase1, np.linspace(-160e6, 0e6,101), seq=seq, plot_seqs=False, postseq = post_seq,
                                                                 proj_func='phase')
            spec.measure()
    #        plt.close()
            shift = spec.fit_params['freq'].value
...
#    from scripts.single_qubit import rabi
    ...
    post_seq = None
    tr_e = rabi.Rabi(qubit2_info, [qubit2_info.pi_amp,], histogram=True, title='|e>',
                                 readout = 'readout_IQ', postseq= post_seq
                   )
    tr_e.measure()
    tr_g = rabi.Rabi(qubit2_info, [0.001,], histogram=True, title='|g>',
                                 readout = 'readout_IQ', postseq= post_seq
                   )
    tr_g.measure()
#    tr = rabi_mixer.Rabi_mixer(qubit_info, [qubit_info.pi_amp/2,], histogram=True, title='|g>+|e>',
##                               readout = 'readout_IQ'
#                   )
#    tr.measure()
    ...
    print(('g amp ', np.abs(g_average), ', g phase ', np.angle(g_average,deg=True)))
    print(('e amp ', np.abs(e_average), ', e phase ', np.angle(e_average,deg=True)))
...
#                               np.linspace(-0.085, 0.085, 81), selective=True,
#                               np.linspace(-0.03, 0.03, 81), selective=True,
        #                       np.linspace(-0.47,-0.41, 81), selective=False,                   
                               ...
                               proj_func='amplitude',readout = 'readout_IQ')
    tr.measure()
...
#    postseq = sequencer.Sequence(qubit_info.rotate(np.pi, 0))
#    postseq = sequencer.Sequence(qubit_info.rotate_selective(np.pi, 0))
#    navgs = [1000,10000,50000,100000,150000, 200000]
#    nsamples = [500, 1000, 2000, 2100]
    for i in range(1):
#        dig.do_set_naverages(navgs[i])
#        for j in range(len(nsamples)):
            
        
#        dig.do_set_nsamples(nsamples[j])
        tr = rabi_mixer.Rabi_mixer(qubit2_info, mixer_info1, mixer_info2,
                                   np.linspace(-0.8, 0.8, 81), selective=False,
#                                   np.linspace(-0.6, 0.6, 101), selective=False,
#                                   np.linspace(-0.08, 0.08, 81), selective=True,
#                                   np.linspace(-0.13, 0.13, 81), selective=True,
            #                       np.linspace(-0.47,-0.41, 81), selective=False,                   
                                   plot_seqs=False, generate=True, repeat_pulse=1, seq=seq,# fix_period = 0.88,#postseq = postseq,
                                   update=True, #extra_info=[qubit_info],
                                   proj_func='amplitude',)# readout = 'readout_IQ')
        tr.measure()
#            plt.title('naverages = %s, nsamples = %s'%(navgs[i],nsamples[j]))
    if update_proj:
        if np.abs(tr.avg_data[np.argmax(abs(tr.avg_data))] - tr.avg_data[len(tr.amps)/2]) < np.abs(tr.avg_data[np.argmin(abs(tr.avg_data))] - tr.avg_data[len(tr.amps)/2]):
            readout.set_IQg(tr.avg_data[np.argmax(abs(tr.avg_data))])
            readout.set_IQe(tr.avg_data[np.argmin(abs(tr.avg_data))])
...
if 1: # ef Power Rabi-Calibrate pi pulse mixer
...
#                               np.linspace(-0.3, 0.3, 81), selective=False,
                               np.linspace(-0.7, 0.7, 81), selective=False,
#                               np.linspace(-0.35, 0.35, 81), selective=True,
#                               np.linspace(-0.12, 0.12, 81), selective=True,
        #                       np.linspace(-0.47,-0.41, 81), selective=False,                   
                               plot_seqs=False, generate=True, repeat_pulse=1, seq=seq,postseq = postseq, fix_period = .7147,
                               update=True, extra_info=qubit2_info,
                               proj_func='phase')
    tr.measure()
...
if 0: # Time Rabi
    from scripts.single_qubit import timerabi_mixer
    tr = timerabi_mixer.TimeRabi_Mixer(qubit2_info,mixer_info1, mixer_info2, np.linspace(1, 200, 51), amp=0.407, proj_func = 'projection')
...
#                                         np.concatenate((np.linspace(5e3, 5e3, 50), np.linspace(6e3, 6e3, 51))),
                                         double_exp=False, generate=True, plot_seqs=False, proj_func='phase', seq=None, readout = 'readout_IQ')    
    t1.measure()
    bla
...
if 0: # T1 mixer with CW stark shift tone

    from .single_qubit import T1_measurement_mixer_CWstark
    pi_amp = .3
    SS_mixer_info2_set.set_pi_amp(pi_amp)
...
#            ])
    ...
    t1 = T1_measurement_mixer_CWstark.T1Measurement_mixer_CWstark(qubit2_info, mixer_info1,mixer_info2, SS_mixer_info,#np.linspace(0, 500e3, 101),
                                         np.linspace(0e3, 6e3, 101),
#                                         np.concatenate((np.linspace(5e3, 5e3, 50), np.linspace(6e3, 6e3, 51))),
                                         double_exp=False,postseq = post_seq, generate=True, plot_seqs=False, proj_func='phase', seq=None)    
    t1.measure()
    bla
...
#    seq = sequencer.Join([sequencer.Trigger(250), qubit2_info.rotate(np.pi, 0)])
    for i in range(1):
        t2 = T2measurement.T2Measurement(qubit_info, np.linspace(0, 0.02e3, 11), detune=60e6, double_freq=False, generate=True, 
                                         seq=None, postseq=None, proj_func='phase',plot_seqs = True, readout = 'readout_IQ') #extra_info=[qubit2_info])
...
#    seq = sequencer.Join([sequencer.Trigger(250), qubit2_info.rotate(np.pi, 0)])
    for i in range(1):
        t2 = T2measurement_mixer.T2Measurement_mixer(qubit2_info, mixer_info1, mixer_info2, np.linspace(0, 30e3, 101), 
                                                     ...
                                                     seq=None, postseq=None,plot_seqs = True, proj_func='amplitude') #extra_info=[qubit2_info])
#        t2 = T2measurement.T2Measurement(qubit_info, np.concatenate((np.linspace(0.1e3, 2.6e3, 81), np.linspace(2.61e3, 10e3, 81))), detune=0.5e6, double_freq=False, generate=True, 
#                                         seq=seq, extra_info=ef_info, postseq=None, proj_func='phase')
...
#    fields = [-0.005, 0.0025, -0.001,0.0005,-0.00025, 0]
...
if 0:#ramsey mixer sigma_x and sigma_y
    for i in range(1):
#        dig.do_set_naverages(avgs[i])
...
if 0: # cw ramsey mixer
    ...
    field = 0.05
    pi_amps = np.linspace(0.1,0.1,1)
    repeat = 1
    SS_mix = 1
    
    ro_freq = 10.8047e9
    cav_disps = np.linspace(10.803e9,10.805e9,11)
#    power = 10
#    mixer_info1_set.set_pi_amp(0.12)
#    mixer_info2_set.set_pi_amp(0.12)
#    readout_info.rfsource1.set_frequency(ro_freq - mixer_info1.deltaf)
#    readout_info.rfsource1.set_power(power)
    ...
    for i in range(len(pi_amps)):
        cav_disp = cav_disps[i]
        if SS_mix == 2:
            SS_mixer_info2_set.set_pi_amp(pi_amps[i])
        ...
        if SS_mix == 1:
            ...
            SS_mixer_info = mclient.get_qubit_info('SS_mixer_info1')
        
        for j in range(repeat):
            t2 = cw_ramsey_mixer.CW_Ramsey_Mixer(qubit2_info,  SS_mixer_info, mixer_info1,mixer_info2, 
...
#        plt.plot(pi_amps,np.mean(df, axis = 1),label = 'df')
        plt.plot(cav_disps,np.mean(df, axis = 1),label = 'df')
        plt.legend()   
        plt.figure()
        plt.title('field = %s'%(field))#%(points[u]))
#        plt.plot(pi_amps,np.mean(tau, axis = 1),label = 'tau')
        plt.plot(cav_disps,np.mean(tau, axis = 1),label = 'tau')
        plt.legend() 
        
        plt.figure()
        plt.plot(10157 - np.mean(df, axis = 1), 1/np.mean(tau, axis = 1))
        plt.scatter( 10157 - 9890, 1/1.62)
        plt.xlim(0,4000)
        plt.ylim(0,6)
    bla
    ...
    for idelay, delay in enumerate(Delays):
        for i in range(repeat):
            t2 = photon_ramsey_measurement.Photon_Ramsey_Measurement_mixer(qubit_info, qubit2_info, SS_mixer_info1, mixer_info1,mixer_info2, 
                                                             np.linspace(0,.2e3,101), detune=60e6, fix_phi0 = -1.8, delay = delay , qubit_pulse = True, double_freq=False, generate=True, 
                                                             seq=None, postseq=None, proj_func='projection', plot_seqs =False) #extra_info=[qubit2_info])
            t2.measure_keysight()
...
#    seq = sequencer.Sequence([sequencer.Trigger(250), qubit_info.rotate_selective(np.pi, 0)])
    postseq = sequencer.Sequence(qubit2_info.rotate(np.pi, 0))
#    postseq = sequencer.Sequence(qubit_info.rotate_selective(np.pi, 0))
    spec = ssbspec_mixer.SSBSpec_mixer(ef2_info,mixer_info1, mixer_info2, np.linspace(-50e6, 50e6, 101), 
                                seq=seq, postseq = postseq, extra_info=qubit2_info, plot_seqs=False, generate=True, proj_func='phase')
    spec.measure_keysight()
#    fit_freq=spec.center
#    deltaf_1 = ef2_info_set.get_deltaf()
#    ef2_info_set.set_deltaf(deltaf_1+fit_freq*1e6)
...
#        seq = sequencer.Trigger(600)
        ...
        t2 = T2measurement_mixer.T2Measurement_mixer(qubit2_info, mixer_info1, mixer_info2, np.linspace(0, 1e3, 101), 
                                                     detune=10e6, double_freq=True, generate=True,# echotype = 'HANN',
...
#                                         seq=seq, extra_info=ef_info, postseq=None, proj_func='phase')
    ...
    plt.figure()
    plt.plot(list(range(len(freq_ge))),freq_ge,label = 'ge freq')
    plt.plot(list(range(len(freq_ef))),freq_ef,label = 'ef freq')
    plt.legend()
    bla
...
#    seq = sequencer.Join([sequencer.Trigger(250), cool, sequencer.Delay(500)])
#    dig.set_naverages(4000)
    efr = efrabi_mixer.EFRabi_mixer(qubit2_info, ef2_info, mixer_info1, mixer_info2, np.linspace(-0.6, 0.6, 101), first_pi=True,second_pi=True, seq=None, generate=True, update=True,
                            proj_func='projection')
    efr.measure_keysight()
    period = efr.fit_params['period'].value
    dig.set_naverages(30000)
    efr = efrabi_mixer.EFRabi_mixer(qubit2_info, ef2_info, mixer_info1,  mixer_info2, np.linspace(-0.6, 0.6, 101), first_pi=False, second_pi=True, selective=False, seq=None, generate=True,
...
#    powers = [4,4,4]
    ...
    if freqs[0] == freqs[1]:
        freqs = np.asarray(list(range(len(freqs))))
