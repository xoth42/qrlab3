...
class CavT1_switch(Measurement1D):

    def __init__(self, qubit_info, cav_info, fwm_gen, switch_channel, disp, delays, proj_num, seq=None, postseq=None, extra_info=None, bgcor=False,
        ...
        if bgcor:
            npoints *= 2
        super(CavT1_switch, self).__init__(npoints, infos=(qubit_info, cav_info), **kwargs)
...
#                    s.append(Delay(5000))
    ...
    def get_ys(self, data=None):
        ys = super(CavT1_switch, self).get_ys(data)
        if self.bgcor:
            return ys[::2] - ys[1::2]
        return ys
