...
def fit_amprabi(params, x, data):
    est = params["ofs"].value - params["amp"].value * np.cos(
        2 * np.pi * x / params["period"].value + params["phase"].value
    )
    return data - est
...
def fit_amprabi_func(params, x, data, meas):
    coeffs = np.polyfit(
        [0, params["pi2_amp"].value, params["pi_amp"].value], [0, np.pi / 2, np.pi], 2
    )
...
def analysis(meas, data=None, fig=None):
    ...
    if meas.fit_type == FIT_AMPFUNC:
        ...
        # stderr is None when lmfit can't compute the covariance (e.g. fit
        # against degenerate all-zero data); fall back to 0 for display.
        amp_stderr = result.params["amp"].stderr or 0
        period_stderr = result.params["period"].stderr or 0
        txt = "Amp = %.03f +- %.03e\nPeriod = %.03f +- %.03e\nPi amp = %.06f" % (
    ...
    print(
        (
            (11 * np.pi - result.params["phase"].value)
            * result.params["period"].value
            / (2 * np.pi)
        )
    )  # Chen 4/3
        ...
        **kwargs,
        ...
        if cancel_info is not None:
            super(Rabi, self).__init__(
                len(amps), infos=(qubit_info, cancel_info), **kwargs
            )
        else:
            if self.heralding:
            ...
            if self.cancel_info is not None:
                for j in range(self.repeat_pulse):
                    s.append(
                        Combined(
                            [
                                self.qubit_info.rotate(0, self.r_axis, amp=amp),
                ...
                if self.selective == 1:
                    s.append(
                        Repeat(
                            self.qubit_info.rotate_selective(0, self.r_axis, amp=amp),
                            self.repeat_pulse,
                        )
                    )
                    ...
                    s.append(
                        Repeat(
                            self.qubit_info.rotate_quasilective(
                                0, self.r_axis, amp=amp
                            ),
                            self.repeat_pulse,
                        )
                    )
    ...
    def analyze(self, data=None, fig=None):
        if self.fit_type == FIT_PARABOLA:
            if self.repeat_pulse % 2 == 0:
                self.pi_amp = 0
                self.pi2_amp = self.analyze_parabola(
                    data=data, fig=fig, xlabel="Amplitude", ylabel="Signal"
                )
            else:
                self.pi_amp = self.analyze_parabola(
                    data=data, fig=fig, xlabel="Amplitude", ylabel="Signal"
                )
                self.pi2_amp = 0
        ...
        if self.update_ins:
            print(
                "Setting qubit pi-rotation ampltidue to %.06f, pi/2 to %.06f"
                % (self.pi_amp, self.pi2_amp)
            )
            if self.selective == 1:
                if self.pi_amp:
                    mclient.instruments[self.qubit_info.insname].set_pi_amp_selective(
                        self.pi_amp
                    )
                if self.pi2_amp:
                    mclient.instruments[self.qubit_info.insname].set_pi2_amp_selective(
                        self.pi2_amp
                    )
            elif self.selective == 0.5:
                if self.pi_amp:
                    mclient.instruments[
                        self.qubit_info.insname
                    ].set_pi_amp_quasilective(self.pi_amp)
                if self.pi2_amp:
                    mclient.instruments[
                        self.qubit_info.insname
                    ].set_pi2_amp_quasilective(self.pi2_amp)
            else:
                if self.pi_amp:
                    mclient.instruments[self.qubit_info.insname].set_pi_amp(self.pi_amp)
                if self.pi2_amp:
                    mclient.instruments[self.qubit_info.insname].set_pi2_amp(
                        self.pi2_amp
                    )
