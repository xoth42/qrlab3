...
def analysis(meas, data=None, fig=None):
    ...
    if meas.double_freq == False:
        fig.axes[0].plot(xs/1e3, -t2_fit_tilted(result.params, xs, 0), label='Fit, tau=%.03f us, df=%.03f kHz'%(result.params['tau'].value/1000, result.params['freq'].value*1e6))
    fig.axes[0].legend()
    fig.axes[0].set_ylabel('Intensity [AU]')
    fig.axes[0].set_xlabel('Time [us]')
    fig.axes[1].plot(xs/1e3, t2_fit_tilted(result.params, xs, ys), marker='s')
    fig.canvas.draw()
...
class GFT2Measurement(Measurement1D):
    ...
    def __init__(self, ge_info, ef_info, delays, detune=0, echotype=ECHO_NONE, necho=1, double_freq=False, seq=None, postseq=None, laser_power = None, **kwargs):
