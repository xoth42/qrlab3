...
def analysis(meas, data=None, fig=None):
    ...
    y2s = y2d[:,1]
    fig.axes[0].clear()   
    fig.axes[0].plot(xs/1e3, y1s, 'bs', ms=3)
    fig.axes[0].plot(xs/1e3, y2s, 'rs', ms=3)    
...
class T1_FT1_Interleaved(Measurement1D):

    def __init__(self, qubit_info, ef_info, delays, double_exp=False, seq=None,
                 postseq=None, **kwargs):
...
#        self.xs = delays / 1e3
        ...
        super(T1_FT1_Interleaved, self).__init__(len(delays)*2, infos=(qubit_info, ef_info), **kwargs)
...
#        self.data.set_attrs(laser_power =self.laser_power)
    ...
    def generate(self):
        ...
        for i, dt in enumerate(self.delays):
            ...
            '''Do a T1 measurement'''
            s_temp = [Combined([
                    Constant(250, -0.033, chan='5m1'),
                    self.seq])]
    
            s_temp += [Combined([
                    Constant(80, -0.033, chan='5m1'),
                    r(np.pi, 0)])]

            s_temp += [Constant(int(dt), -0.033, chan='5m1')] 

            if self.postseq is not None:
                s_temp += [self.postseq]

            s_temp += [Combined([
                    Constant(self.readout_info.pulse_len, -0.033, chan='5m1'), 
                    Constant(self.readout_info.pulse_len, 1, chan=self.readout_info.readout_chan),
                    Constant(self.readout_info.pulse_len, 1, chan=self.readout_info.acq_chan),
                    ])]
    
            s_temp += [Constant(2000, -0.033, chan='5m1')]
            
            '''Do an FT1 measurement'''
            s_temp = [Combined([
                    Constant(250, -0.033, chan='5m1'),
                    self.seq])]
