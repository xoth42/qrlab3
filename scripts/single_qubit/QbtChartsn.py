...
if 0: # Calibrate pi pulse
    ...
    for i in range(8):
#        seq = sequencer.Join([sequencer.Trigger(250), sequencer.Repeat(qubit_info.rotate(np.pi, 0), 20)])
        tr = rabi.Rabi(qubit_info, np.linspace(-0.7, 0.7, 81), plot_seqs=False, update=False, seq=None, selective=False, singleshotbin=False)#, fix_period=0.00924)
        tr.measure()
        tr = rabi.Rabi(qubit_info, np.linspace(-0.005, 0.005, 81), plot_seqs=False, update=False, seq=None, selective=1)
        tr.measure()
    stop
...
if 0: #EF rabi
    ...
    for reprate in [2000]:
        ...
        efr = efrabi.EFRabi(qubit_info, ef_info, np.linspace(-0.7, 0.7, 61), seq=None, second_pi=True)
        efr.measure()

        period = efr.fit_params['period'].value
        alz.set_naverages(10000)
        efr = efrabi.EFRabi(qubit_info, ef_info, np.linspace(-0.7, 0.7, 61), first_pi=False, force_period= period)
        efr.measure()
    bla
...
#    seq = sequencer.Join([sequencer.Trigger(250), cavity_info1A.rotate(3.0, 0), cavity_info1B.rotate(3.0, 0)])
    t1 = T1measurement.T1Measurement(qubit_info, np.concatenate((np.linspace(0, 80e3, 41), np.linspace(81e3, 350e3, 61))), double_exp=False,
                                     generate=True, seq=None)
    t1.measure()
...
#        seq = sequencer.Join([sequencer.Trigger(250), cavity_info1A.rotate(0.01, 0)])#, cavity_info1B.rotate(1.0, 0)])
        t2 = T2measurement.T2Measurement(qubit_info, np.linspace(0, 20e3, 121), detune=600e3, singleshotbin=False, double_freq=False,
                                         )#singleshotbin=False, Qswitch_infoA=Qswitch_info1A, Qswitch_infoB=Qswitch_info1B,
...
if 0: # T2echo
    for j in range(1):
        from scripts.single_qubit import T2measurement
        t2 = T2measurement.T2Measurement(qubit_info, np.linspace(0.5e3, 50e3, 101), detune=300e3,
                                         echotype = T2measurement.ECHO_HAHN, necho=1, generate=True)
        t2.measure()
    start
...
if 0: # EFT2
    ...
    eft2 = EFT2measurement.EFT2Measurement(qubit_info, ef_info, np.linspace(0.5e3, 25e3, 101), detune=600e3, double_freq=False)#, echotype = EFT2measurement.ECHO_HAHN)
    eft2.measure()

if 1: # GFT2
    from scripts.single_qubit import GFT2measurement
    gft2 = GFT2measurement.GFT2Measurement(qubit_info, ef_info, np.linspace(0.5e3, 10e3, 101), detune=1000e3, double_freq=False)#, echotype = GFT2measurement.ECHO_HAHN)
    gft2.measure()
    bla

if 0: #number splitting:
    from scripts.single_qubit import spectroscopy
    seq = sequencer.Join([sequencer.Trigger(250), cavity_infoA.rotate(np.pi, 0)])
#    postseq = sequencer.Sequence([sequencer.Trigger(250), cavity_info.rotate(np.pi, 0)])
    ...
    spec = spectroscopy.Spectroscopy(mclient.instruments['brick2'], qubit_info, np.linspace(qubit_freq-8e6, qubit_freq+2e6, 101), [11.5],
                                     plen=6000, seq = seq, amp=0.09, extra_info=cavity_infoA, plot_seqs=True)
    spec.measure()


if 0: #Sideband modulated number splitting:
    from scripts.single_qubit import ssbspec
    seq = sequencer.Join([sequencer.Trigger(250), cavity_infoA.rotate(np.pi*2, 0)])
...
if 1: #mixer calibration:
    ...
    cal = mixer_cal('qubit1ef', 5000.506e6-277.152e6, 'VA', 'AWG2', verbose=True,
                        base_amplitude= 4,
                        va_lo='brick4') # The frequency is the targeted lower sideband frequency, not the carrier
