# -*- coding: utf-8 -*-
"""
Created on Tue Dec 08 18:15:13 2020
...
#    labels = ['without qubit, delay %s'%(meas.delay),'without qubit', 'with qubit','with qubit, delay %s'%(meas.delay)]
    ...
    #    result2 = lmfit.minimize(t2_fit, result.params, args=(xs,ys))
        ...
        if meas.double_freq == False:
            fig.axes[0].plot(xs/1e3, -changing_freq_fit(result.params, xs, 0), 
                    label='%s, Fit, tau=%.03f us, df=%.03f kHz, df_final = %.03f kHz,\n df_average = %.03f kHz\n tau_i = %.04f us, tau_f = %.04f us, tau_ave = %.04f us'%(
                            labels[i],
    ...
    fig = plt.figure()
    fig.add_subplot(111)
    off = (return_result[0]['ofs'].value + return_result[1]['ofs'].value)/2
    amp = (return_result[0]['amp'].value + return_result[1]['amp'].value)/2
    ys_norm = (ys_als - off) / amp             
    fig.axes[0].plot(xs/1e3, ys_norm[0], 'bs-', ms=3, label = labels[0])
    fig.axes[0].plot(xs/1e3, ys_norm[1], 'ys-', ms=3, label = labels[0])
    fig.axes[0].plot(xs/1e3, np.sqrt(ys_norm[0] **2 + ys_norm[1]**2), 'gs-', ms=3, label = 'total')
...
class Ramsey_Measurement_mixer_xy(Measurement1D):
...
#        self.xs = delays / 1e3        # For plotting purposes
        self.xs = np.array([delays,delays]).transpose().flatten() / 1e3
        self.detune = detune
        ...
        super(Ramsey_Measurement_mixer_xy, self).__init__(len(self.xs), infos=(qubit_info,SS_mixer_info1, mixer_info1,mixer_info2,), **kwargs)
...
#            s.append(Join(s_temp))
            ...
            s.append(self.seq)
#            s.append(Constant(10, 1, chan=self.readout_info.acq_chan))
            s.append(r(np.pi/2, Y_AXIS))#s.append(Pad(r(np.pi/2, X_AXIS), 250, PAD_LEFT))
