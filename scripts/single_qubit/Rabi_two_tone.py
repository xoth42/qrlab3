...
class Rabi_two_tone(Measurement1D):

    def __init__(self, qubit_info, qubit2_info, amps, update=False, seq=None, r_axis=0, fix_phase=True,
        ...
        super(Rabi_two_tone, self).__init__(len(amps), infos=(qubit_info, qubit2_info), **kwargs)
    ...
    def generate(self):
        ...
        for i, amp in enumerate(self.amps):
            ...
            if self.selective==1:
                s.append(Combined([
                        Repeat(self.qubit_info.rotate_selective(0, self.r_axis, amp=amp), self.repeat_pulse),
                        Repeat(self.qubit2_info.rotate_selective(0, self.r_axis, amp=amp*.6), self.repeat_pulse)
                        ]))
            elif self.selective==0.5:
                s.append(Combined([
                        Repeat(self.qubit_info.rotate_quasilective(0, self.r_axis, amp=amp), self.repeat_pulse),
                        Repeat(self.qubit2_info.rotate_quasilective(0, self.r_axis, amp=amp*.6), self.repeat_pulse)
                        ]))
            else:
                s.append(Combined([
                        Repeat(self.qubit_info.rotate(0, self.r_axis, amp=amp), self.repeat_pulse),
                        Repeat(self.qubit2_info.rotate(0, self.r_axis, amp=amp*.6), self.repeat_pulse)
                        ]))
