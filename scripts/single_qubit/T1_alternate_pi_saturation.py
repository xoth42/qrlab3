...
def analysis(meas, data=None, fig=None):
    ...
    fig.axes[0].plot(xs/1e3, y2s, 'rv', ms=3)
...
class T1Measurement(Measurement1D):
    ...
    def __init__(self, qubit_info, delays, double_exp=False, seq=None,
        ...
        super(T1Measurement, self).__init__(len(delays)*2, infos=qubit_info, **kwargs)
...
#            s.append(r(np.pi, 0))
            s.append(Combined([
                    Constant(100000, 0.3, chan=self.qubit_info.sideband_channels[0]),
                    Constant(100000, 0.3, chan=self.qubit_info.sideband_channels[1]),
            ]))
            s.append(Delay(dt))
