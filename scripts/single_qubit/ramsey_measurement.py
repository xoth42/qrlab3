...
def changing_freq_fit(params, x, data):
    '''
    Exponentially decaying sine fit function: a + b * exp(-c * x) * sin((d + slope * x)* x + e)
    ...
    sine = np.sin(2 * np.pi * x * (params['freq'].value + params['slope'].value * x ) + params['phi0'].value)
    exp = np.exp(-(x / params['tau'].value))
    est = params['ofs'].value + params['amp'].value * exp * sine
    return data - est
...
def analysis(meas, data=None, fig=None):
    ...
    params.add('slope', value=0.00001,vary = True)
...
#    result2 = lmfit.minimize(t2_fit, result.params, args=(xs,ys))
    ...
    if meas.double_freq == False:
        fig.axes[0].plot(xs/1e3, -changing_freq_fit(result.params, xs, 0), label='Fit, tau=%.03f us, df=%.03f kHz, df_final = %.03f kHz'%(result.params['tau'].value/1000, result.params['freq'].value*1e6,result.params['freq'].value*1e6 + result.params['slope'].value *xs[-1] * 1e6))
        fig.axes[0].legend()
