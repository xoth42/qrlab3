...
class Rabi_singlequbit_interleaved(Measurement1D):

    def __init__(self, qubit_info, qubit_info2, qubit2_info, amps, update=False, seq=None, seq2 =None, r_axis=0, fix_phase=True, rel_amp=1.0, rel_angle=0.0,
        ...
        self.amps = amps
#        self.xs = amps
        self.xs = np.array([amps,amps]).transpose().flatten() / 1e3      # For plotting purposes
        ...
        super(Rabi_singlequbit_interleaved, self).__init__(len(amps)*2, infos=(qubit_info,qubit_info2, qubit2_info), **kwargs)
    ...
    def generate(self):  #That is the original generate function 
        ...
        for i, amp in enumerate(self.amps):
            ...
            if self.selective==1: #Don't work yet
                ...
                s.append(Combined([self.qubit_info.rotate(0, 0, amp=amp), self.qubit_info2.rotate(0, self.rel_angle, amp=amp*self.rel_amp)]))
