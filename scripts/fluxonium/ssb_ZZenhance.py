...
class SSBSpec(Measurement1D):
    ...
    def __init__(self, qubit_info, ZZ_info, detunings, plen =500, sigma=6, amp=0.2, phase=0,seq=None, postseq=None, bgcor=False, coplay_delay=0, **kwargs):
...
#        for df in self.detunings:
            ...
            s.append(Join([
                self.seq, (Combined([GaussSquare(int(self.plen), ampI, self.sigma, chan=chs[0]),
                          GaussSquare(int(self.plen), ampQ, self.sigma, chan=chs[1])])),
                g(),
            ]))
