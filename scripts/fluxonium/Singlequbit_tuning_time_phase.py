...
class Singlequbit_tuning_time_phase(Measurement2D):

#The purpose here is to sweep over time and detuning for the combined pulse without the pi pulse on the control qubit

    def __init__(self, qubit_info, qubit_info2, qubit2_info, times, rel_phases, amp=0.35, phase=0, rel_amp=1, sigma=5, 
        ...
        super(Singlequbit_tuning_time_phase, self).__init__(npoints, infos=(qubit_info,qubit_info2, qubit2_info), **kwargs)
...
#                if plen > 0:
        ...
        #                GaussSquare(int(plen), ampQc, self.sigma, chan=chs2[1]),            
                        ...
                        Constant(int(plen), self.amp *self.rel_amp * np.cos(self.phase+rel_phases), chan=chs2[0]),
                        Constant(int(plen), self.amp *self.rel_amp * np.sin(self.phase+rel_phases), chan=chs2[1]),              
                    ]))
                s.append(g)
#                s.append(g)
                if self.postseq:
...
#                if plen > 0:
    ...
    def get_ys(self, data=None):
        ys = super(Singlequbit_tuning_time_phase, self).get_ys(data)
        return ys
