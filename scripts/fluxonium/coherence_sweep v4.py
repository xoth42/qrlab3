...
qubit1ge.set_deltaf(qubit_info.deltaf + spec.center *1e6)
qubit_info = mclient.get_qubit_info('qubit1ge')


tr = rabi.Rabi(qubit_info, np.linspace(-0.8, 0.8, 51), plot_seqs=False, generate=True, selective=False, repeat_pulse=1, seq=None,
...
#        t1 = T1measurement.T1Measurement(qubit_info, np.concatenate(np.linspace(0.1e3, 1.9e3, 19), np.linspace(2e3, 20e3, 36), np.linspace(22e3, 550e3, 60)), double_exp=True, generate=True, plot_seqs=False, proj_func='phase')
        ...
        qubit1ge.set_deltaf(qubit_info.deltaf + fxn_freq1D[len(fxn_freq1D)-1] - fxn_freq1D[len(fxn_freq1D) - 2])
        qubit_info = mclient.get_qubit_info('qubit1ge')
