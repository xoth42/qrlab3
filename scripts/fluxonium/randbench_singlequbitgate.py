...
class randbench_singlequbitgate(Measurement1D):

    def __init__(self, qubit_info, qubit_info2, num_cal_points, n_gates_start, n_gates_stop, n_gates_step, seq=None, postseq=None, **kwargs):
        ...
        super(randbench_singlequbitgate, self).__init__(n_points, infos=(qubit_info,qubit_info2), **kwargs)
...
#########   Combined([r(np.pi, 0),r2(np.pi,0)])                      
        for j in range(self.num_cal_points):
            s.append(self.seq)
            temp_seq = Sequence()
            temp_seq.append(Combined([r2(np.pi, X_AXIS), r(np.pi, X_AXIS)]))   
...
#                    s.append(Delay(5))
                if rotation_list[i] in ['B3', 'B7']:
                    temp_seq.append(Combined([r2(np.pi, X_AXIS), r(np.pi, X_AXIS)]))
                    temp_seq.append(Combined([r2(np.pi, Y_AXIS), r(np.pi, Y_AXIS)]))
#                    s.append(Delay(5))
                if i%100==99:
                    s.append(Join(temp_seq))
                    temp_seq = Sequence()
...
#                s.append(self.postseq)
    ...
    def seq_table(self, symbol):
        ...
        r2 = self.qubit_info2.rotate
        if symbol == 'A1': # X PI/2
            return (Combined([r2(np.pi/2, X_AXIS),r(np.pi/2,X_AXIS)]))
        elif symbol == 'A2': # Y PI/2
            return (Combined([r2(np.pi/2, Y_AXIS),r(np.pi/2, Y_AXIS)]))
        elif symbol == 'A3': #X -PI/2
            return (Combined([r2(-np.pi/2, X_AXIS),r(-np.pi/2,X_AXIS)]))
        elif symbol == 'A4': #Y -PI/2
            return (Combined([r2(-np.pi/2, Y_AXIS),r(-np.pi/2, Y_AXIS)]))
        elif symbol == 'B1': # X PI
            return (Combined([r2(np.pi, X_AXIS),r(np.pi,X_AXIS)]))
        elif symbol == 'B2':# Y PI
            return (Combined([r2(np.pi, Y_AXIS),r(np.pi, Y_AXIS)]))
        elif symbol == 'B3':            #Z(PI)
            return Delay((self.qubit_info.w)*4)
#        elif symbol == 'B3':            #Z(PI)
#            return Delay(40)
        elif symbol == 'B4':            #Identity
            return Delay((self.qubit_info.w)*4)
            ...
            return Delay((self.qubit_info.w)*4)            #Z(-PI)
