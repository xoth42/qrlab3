...
def ssb(qubit_info, seq, extra_info): #can do single or double fits depending on selection 
    spec = ssbspec.SSBSpec(qubit_info, np.linspace(-5e6, 10e6, 81), proj_func='phase', seq=seq, extra_info=extra_info)
    spec.measure()
...
#
def ZZ_tune(qubit_info, power_range, freq_range, extra_info):
    fig, ax = plt.subplots(nrows=1, ncols=1)
    seq = sequencer.Join([sequencer.Trigger(250), cool, sequencer.Delay(150), extra_info.rotate(np.pi,0)])
    ZZ.set_rf_on(True)
    for power in power_range:
       ...
       for freq in freq_range:
           ...
           L_pi.append(ssb(qubit_info, seq, extra_info=extra_info))
         ...
         #  plt.plot(freq_range,L_nopi)
          # plt.plot(freq_range,L_pi)
           ax.plot(freq,L_nopi[-1], marker='o', fillstyle='none', color='r')
           ax.plot(freq,L_pi[-1], marker='o', fillstyle='none', color='b')
           fig.canvas.draw()
#
...
if 0:
    #T2R on qubit1
    t2 = T2measurement.T2Measurement(gate_info1, np.linspace(10, 1e3, 81), detune=2e6, plot_seqs = False, generate=True,
                                     proj_func='phase', seq=seq_cool, postseq=None, extra_info = gate_info2)
    t2.measure()
    T2R_list_1.append(t2.fit_params['tau'].value)
    T2R_list_1_err.append(t2.fit_params['tau'].stderr)
    #T2R on qubit 2
    t2 = T2measurement.T2Measurement(gate_info2, np.linspace(10, 1e3, 81), detune=2e6, plot_seqs = False, generate=True,
                                     proj_func='phase', seq=seq_cool, postseq=None, extra_info = gate_info2)
    t2.measure()
    T2R_list_2.append(t2.fit_params['tau'].value)
    T2R_list_2_err.append(t2.fit_params['tau'].stderr)
    ...
    t2.measure()
    T2E_list_1.append(t2.fit_params['tau'].value)
    T2E_list_1_err.append(t2.fit_params['tau'].stderr)
    ...
    t2.measure()
    T2E_list_2.append(t2.fit_params['tau'].value)
    T2E_list_2_err.append(t2.fit_params['tau'].stderr)

    #T1 on qubit 1
    t1 = T1measurement.T1Measurement(gate_info1, np.linspace(0, 50e3, 81), double_exp=False, generate=True, plot_seqs=False,
                                         proj_func='phase', seq=seq_cool, postseq=None, extra_info=gate_info2)  #Note the rep time is 50us
    t1.measure()
    T1_list_1.append(t1.fit_params['tau'].value)
    T1_list_1_err.append(t1.fit_params['tau'].stderr)



    #T1 on qubit 2
    t1 = T1measurement.T1Measurement(gate_info2, np.linspace(0, 50e3, 81), double_exp=False, generate=True, plot_seqs=False,
                                         proj_func='phase', seq=seq_cool, postseq=None, extra_info=gate_info2)  #Note the rep time is 50us
    t1.measure()
    T1_list_2.append(t1.fit_params['tau'].value)
    T1_list_2_err.append(t1.fit_params['tau'].stderr)
...
##    Getting some cooling prediction
    ...
    freq_range = 15e6        
    cooling_spec(3520e6, 15e6, qubit2_info2, [2,3,4,5])
###    
    bla
...
if 0:
   ##Optimizing ZZ 
#
#    rabi_test(qubit1ge, qubit_info, seq_cool,np.linspace(-0.2, 0.2, 61) )   
#    rabi_test(qubit2ge, qubit2_info, seq_cool,np.linspace(-0.1, 0.1, 61))   

    ZZ.set_rf_on(True)
    alz.set_naverages(1500)
    power_range = np.linspace(0,0, 1)
    freq_range = np.linspace(3810e6,3870e6, 11)
    ZZ_tune(qubit_info2, power_range, freq_range, extra_info = qubit2_info2)
    bla    
...
#    Rabi checking pi amps from 5/6 for both qubits
    rabi_test(qubit1ge, qubit_info, seq_cool,np.linspace(-0.5, 0.5, 61) )   
...
if 0:
    ...
    rabi_test(qubit2ge_2, qubit2_info2, seq_cool, np.linspace(-0.3, 0.3, 61))   
#    for i in range(1):    
...
#        
    ...
    Drag_test(qubit_info2)
    Drag_test(qubit2_info2)  
