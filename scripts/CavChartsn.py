...
if 0: # Ramsey revival to calibrate wigner tomo
    ...
    rr = RamseyRevival.RamseyRevival(qubit_info, ef_info, np.linspace(0, 2000, 151), 
                                     seq = seq, extra_info = cavity_infoA , readout='readout_IQ')
    rr.measure_keysight()
...
#    disp = 2.5
#    seq = sequencer.Join([sequencer.Trigger(250), cA(disp, 0), 
#                      ge(np.pi/2, 0), sequencer.Delay(277), cA(disp,0), 
#                      geqs(np.pi, 0), 
#                      cA(-disp,0)])
    Wfun = WignerbyParity.WignerFunction(qubit_info, ef_info, cavity_infoA,
                                         xs = np.linspace(-1.2,1.2,21), ys = np.linspace(-1.2,1.2,21),
                                         t_ge=233, t_gf=0,
