...
class grape_timeshift(Measurement1D):
    ...
    def __init__(self, qubit_info, qubit_a4, cavity_info, rel_delays, seq=None, postseq=None, **kwargs):
        self.qubit_info = qubit_info
        self.qubit_a4 = qubit_a4
        ...
        super(grape_timeshift, self).__init__(npoints, residuals=False, infos=[qubit_info, qubit_a4, cavity_info], **kwargs)
    ...
    def generate(self):
        ...
        for dt in self.rel_delays:
            s.append(self.seq)
            if dt < 0:
                s.append(Combined([Join([Constant(int(-dt), 0, chan=self.qubit_info.sideband_channels[0]),
                                         CSVPulse(r'C:\qrlab\pulseseq\CSVPulses\envelope_fock_0_4_transmon_1000ns.csv', 
                                                  44, chan=self.qubit_info.sideband_channels[0])]),
                                   Join([CSVPulse(r'C:\qrlab\pulseseq\CSVPulses\envelope_fock_0_4_cavity_1000ns.csv', 
                                                  144, chan=self.cavity_info.sideband_channels[0]),
                                         Constant(int(-dt), 0, chan=self.cavity_info.sideband_channels[0])])
                        ]))
            elif dt > 0:
                s.append(Combined([Join([CSVPulse(r'C:\qrlab\pulseseq\CSVPulses\envelope_fock_0_4_transmon_1000ns.csv', 
                                                  44, chan=self.qubit_info.sideband_channels[0]),
