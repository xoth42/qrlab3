...
#    rofreq = 6945e6
    freq_range = 2.0e6
    ro = rocavspectroscopy.rocavspectroscopy(qubit_info, np.linspace(2, 9, 1),
...
if 0: # SSB spec
    ...
    for i in range(1):
        ...
        spec = ssbspec.SSBSpec(qubit_info, np.linspace(-5e6, 5e6, 81), seq=seq, plot_seqs=False, proj_func='phase', readout='readout_IQ')
        spec.measure()
#        plt.close()
    bla
...
if 1: # Calibrate pi pulse
    ...
    tr = rabi.Rabi(qubit_info, 
                  np.linspace(-0.4, .4, 151), selective=False,
                   plot_seqs=False, generate=True, repeat_pulse=1, update=False, readout='readout_IQ')
    tr.measure()
    bla
...
if 0: # T1
    ...
    t1 = T1measurement.T1Measurement(qubit_info, np.linspace(0, 120e3, 50), 
                                     double_exp=False, generate=True, plot_seqs=False, readout='readout_IQ')
    t1.measure()
#    bla
...
#    dig.set_naverages(1000)
    efr = efrabi.EFRabi(qubit_info, ef_info, np.linspace(-0.1, 0.1, 51), plot_seqs=False, selective=False, generate=True, postseq = None)
