# -*- coding: utf-8 -*-
"""
Created on Mon May 25 11:09:36 2020
...
class ROCavSpectroscopyfwm_keysight(Measurement1D):
    ...
    def __init__(self, qubit_info, powers, freqs, plot_type=None, qubit_pulse=False, seq=None,  **kwargs):
        ...
        super(ROCavSpectroscopyfwm_keysight, self).__init__(1, infos=(qubit_info,), **kwargs)
    ...
    def generate(self):
        ...
        if self.qubit_pulse:
            s.append(Combined([
                    Join([Delay(self.qubit_info.w-self.readout_info.pulse_len),Constant(self.readout_info.pulse_len, 1, chan=self.readout_info.acq_chan)]),
                    Join([Delay(self.qubit_info.w-self.readout_info.pulse_len-200),Constant(self.readout_info.pulse_len, 1, chan=self.readout_info.readout_chan),Delay(200)]),
                    Join([self.qubit_info.rotate(np.pi, 0)]),
            ]))
#            s.append(Delay(2000))
