# -*- coding: utf-8 -*-
"""
Created on Thu Jan 28 13:58:45 2021
...
def analysis(sweep_freqs, freqs, realdata, imagdata, fig_name, full_fig_name, Sij, fig=None):
    fn = None
    fig = pl.figure()
    a=[0,0,0,0]
    ...
    sweep_freqs = np.concatenate((sweep_freqs, np.zeros(1) + sweep_freqs[1]-sweep_freqs[0] + sweep_freqs[-1]))
    ampdata = np.zeros((len(Sij),len(sweep_freqs), len(freqs)))
    ampdata_a = np.zeros((len(Sij),len(freqs), len(sweep_freqs)))
    xs,ys = np.meshgrid(sweep_freqs/float(1e9), freqs/float(1e9))
    gss=[0,0,0,0]
    ...
    #        ampdata = np.zeros((len(realdata),len(sweep_freqdata[0,:]), len(freqdata[0,:])))
    #    imag = np.zeros((len(self.sweep_freqs),len(self.freqs)))
        for i in range(len(sweep_freqs)-1):
            ampdata[k][i] = 20*np.log10(np.sqrt(realdata[k][i,:]**2 + imagdata[k][i,:]**2))
            
        ampdata[k][len(sweep_freqs)-1] = 20*np.log10(np.sqrt(realdata[k][i,:]**2 + imagdata[k][i,:]**2))
    #    print Z
    ...
    #    pl.colorbar( a[k] )#, ax = gs[k/2, k%2] )
        fig.axes[k].set_xlabel('sweep_freqs(GHz)')
...
class Freq_Sweep_VNA(Measurement1D):
    ...
    def __init__(self, drive_brick, sweep_freqs, freqs, average_factor,avelimit,if_bandwidth, Sij, fig_name,comment, **kwargs):
        self.drive_brick = drive_brick
        self.sweep_freqs = sweep_freqs
#        print 'self.sweep_freqs', self.sweep_freqs
        self.freqs = freqs
        ...
        self.dsweep_freqs = sweep_freqs[1] - sweep_freqs[0]
...
#        self.sweep_freqdata = self.data.create_dataset('sweep_freqs', shape=[1,len(self.sweep_freqs)])
#        self.freqdata = self.data.create_dataset('freqs', shape=[1,len(self.freqs)])
##        self.ampdata = self.data.create_dataset('amplitudes', shape=[len(self.sweep_freqs),len(freqs)])
##        self.phasedata = self.data.create_dataset('phases', shape=[len(self.sweep_freqs),len(freqs)])
#        print self.freqdata.get_fullname()
        self.realdata = [0,0,0,0]
        self.imagdata = [0,0,0,0]
        for i, sij in enumerate(self.Sij):
            
            self.realdata[i] = self.data.create_dataset('real%s'%(sij), shape=[len(self.sweep_freqs),len(freqs)])
            self.imagdata[i]= self.data.create_dataset('imaginary%s'%(sij), shape=[len(self.sweep_freqs),len(freqs)])
#        
...
#        self.sweep_freqdata[0,:] = self.sweep_freqs
        ...
        xs,ys = np.meshgrid(self.sweep_freqs, self.freqs)
...
#        avelimit = int(timelimit/VNA.get_sweep_time())
        ...
        for isweep_freq, sweep_freq in enumerate(self.sweep_freqs):
            if isweep_freq == 0:
                self.drive_brick.set_rf_on(False)
            else:
                self.drive_brick.set_rf_on(True)                
            self.drive_brick.set_frequency(sweep_freq)
            time.sleep(10)
            if self.average_factor[isweep_freq] < avelimit:
                avelimit = self.average_factor[isweep_freq]
            ave = avelimit
            if self.average_factor[isweep_freq] > avelimit:
                VNA.set_average_factor(ave)
            count = 0
    
            while count < self.average_factor[isweep_freq]:
                ave = avelimit
                
                if (self.average_factor[isweep_freq]-count) < avelimit:
                    ave = self.average_factor[isweep_freq]-count
                    VNA.set_average_factor(ave)
...
#                VNA.set_trigger_source('internal')
    ...
    #                    self.freqdata[0,:] = freqs
                #        print freqs
                #        print self.freqdata
                        self.realdata[i][isweep_freq,:] = reals
                        self.imagdata[i][isweep_freq,:] = imags
            
            
                    else:
                        reals =( reals *ave + self.realdata[i][isweep_freq,:] * count)/float(ave+count)
                        imags =( imags *ave + self.imagdata[i][isweep_freq,:] * count)/float(ave+count)
                        self.realdata[i][isweep_freq,:] = reals
                        self.imagdata[i][isweep_freq,:] = imags
        ...
        #            fig.axes[0].title = Sij[0]
                ...
                for i in range(len(self.Sij)):
                    ...
                    self.fig.axes[i].set_xlim(xs.min(), xs.max()+self.dsweep_freqs)
                    self.fig.axes[i].set_ylim(ys.min(), ys.max())
        
            for i in range(len(self.Sij)):
                z1 = 20*np.log10(self.realdata[i][isweep_freq,:]**2 + self.imagdata[i][isweep_freq,:]**2)
                z1 = z1[:,None].T
                z2 = 20*np.log10(self.realdata[i][isweep_freq,:]**2 + self.imagdata[i][isweep_freq,:]**2)
                z2 = z2[:,None].T
                z = np.concatenate([z1,z2])
                z = np.transpose(z)
                x = np.zeros((len(self.freqs),2))
                x[:,0] = self.sweep_freqs[isweep_freq]
                x[:,1] = self.sweep_freqs[isweep_freq]+self.dsweep_freqs
