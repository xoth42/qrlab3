...
class flux_phase_sweep(Measurement1D):

    def __init__(self, qubit_info, powers, currents, current_source=None, plot_type=None, qubit_pulse=False, seq=None,  **kwargs):
        ...
        if plot_type is None:
            if len(powers) > len(currents):
                plot_type = POWER
            else:
                plot_type = SPEC
        self.plot_type = plot_type

        super(flux_phase_sweep, self).__init__(1, infos=(qubit_info,), **kwargs)
        self.data.create_dataset('powers', data=powers)
        self.data.create_dataset('currents', data=currents)
        self.ampdata = self.data.create_dataset('amplitudes', shape=(len(powers),len(currents)))
        self.phasedata = self.data.create_dataset('phases', shape=(len(powers),len(currents)))
       ...
       # alz.set_interrupt(False)
        ...
        for ipower, power in enumerate(self.powers):
            ...
            for icurrent, current in enumerate(self.currents):
                self.current_source.do_set_current(current)
                time.sleep(0.1)
...
#                    return
    ...
    def analyze(self, data=None, ax=None):
        ...
        analysis(self.powers, self.currents, ampdata, self.phasedata, self.plot_type, ax=pax)
