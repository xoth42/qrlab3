...
def analysis(powers, freqs, realdata, imagdata, fig_name, full_fig_name, Sij, fig=None):
    ...
    powers = np.concatenate((powers, np.zeros(1) + powers[1]-powers[0] + powers[-1]))
    ampdata = np.zeros((len(Sij),len(powers), len(freqs)))
    ampdata_a = np.zeros((len(Sij),len(freqs), len(powers)))
    xs,ys = np.meshgrid(powers, freqs/float(1e9))
    gss=[0,0,0,0]
...
#
        super(Power_Sweep_VNA, self).__init__(1, **kwargs)
#        self.set_attrs(
#            title=self.fig_name,
...
#        )
        ...
        self.data.create_dataset('powers', data=self.powers)
...
#        print self.freqdata.get_fullname()
        ...
        for i, sij in enumerate(self.Sij):
            ...
            self.realdata[i] = self.data.create_dataset('real%s'%(sij), shape=[len(self.powers),len(freqs)])
            self.imagdata[i]= self.data.create_dataset('imaginary%s'%(sij), shape=[len(self.powers),len(freqs)])
#        
...
#        self.powerdata[0,:] = self.powers
        ...
        xs,ys = np.meshgrid(self.powers, self.freqs)
