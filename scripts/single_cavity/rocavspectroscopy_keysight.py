...
def S21(params, x, y):
    est = np.sqrt(params["kappa_prod"]) / (
        -1j * (x - params["omega_c"]) - (params["kappa_a"]) / 2.0
    )
...
def analysis(
    powers, freqs, ampdata, phasedata=None, plot_type=POWER, square_amps=False, ax=None
):
    if ax is None:
    ...
    if plot_type == SPEC:
        ...
        for ipower in range(0, len(powers)):
            for ifreq in range(0, len(freqs)):
                max_amp[ipower][ifreq] = (
                    20 * np.log10(ampdata[ipower, ifreq]) - powers[ipower] - 78
                )
        ...
        params.add(
            "kappa_prod", value=(np.max(np.abs(amps)) * 0.5e6) ** 2.001, min=0
        )  # ,vary = False)
        params.add(
            "omega_c",
            value=freqs[np.argmax(np.abs(amps))] * 1.00002,
            min=freqs[np.argmax(np.abs(amps))] * 0.9998,
            max=freqs[np.argmax(np.abs(amps))] * 1.0002,
        )  # ,vary = False)
        ...
        print(
            (
                "total Q: ",
                result.params["omega_c"].value / result.params["kappa_a"].value,
            )
        )

        fitdata = np.sqrt(result.params["kappa_prod"].value) / (
            -1j * (freqs - result.params["omega_c"].value)
            - (result.params["kappa_a"].value) / 2.0
        )
        fitdata = (
            fitdata + result.params["roff"].value + 1j * result.params["ioff"].value
        )
        ax.plot(
            fs / 1000000,
            np.abs(fitdata),
            ...
            label="freq = %s MHz\n kappa = %s MHz"
            % (
                result.params["omega_c"].value / 1e6,
                result.params["kappa_a"].value / 1e6,
            ),
        )
        ax2.plot(
            fs / 1000000,
...
class ROCavSpectroscopy_keysight(Measurement1D):
    def __init__(
        ...
        **kwargs,
        ...
        super(ROCavSpectroscopy_keysight, self).__init__(
            1, infos=(qubit_info,), **kwargs
        )
        ...
        for ipower, power in enumerate(self.powers):
            ...
            for ifreq, freq in enumerate(self.freqs):
                if self.readout is "readout_IQ":
                    ...
                    self.readout_info.rfsource.set_frequency(
                        freq - self.readout_info.deltaf
                    )  # yingying change to readout freq
                else:
                ...
                )  # self.readout is not 'readout_IQ')
