...
def analysis(meas, data=None, fig=None):
    ...
    if meas.bgcor == True:
        vmax = 1    # was 20, set to 0.7 for scaling (juliang)
...
#            ys = xs
        ...
        if bgcor:
            npoints *= 2
        super(WignerFunction, self).__init__(npoints, infos=(qubit_info, cav_info), residuals=False, **kwargs)
...
#                    s.append(ge(np.pi/2, Y_AXIS))
    ...
    def get_ys(self, data=None):
        ys = super(WignerFunction, self).get_ys(data)
        if self.bgcor:
...
#        print(data.shape)   # for debugging (juliang)
