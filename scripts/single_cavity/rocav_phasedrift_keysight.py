...
def analysis(powers, freqs, ampdata, phasedata=None, plot_type=POWER, ax=None):
    ...
    if plot_type == SPEC:
        for ipower, power in enumerate(powers):
            ax1.plot(ampdata[ipower,:], label='Amps, Power %.02f dB'%power)
            ax2.plot(phasedata[ipower,:], label='Phase, Power %.02f dB'%power)
        ax1.legend()
        ax2.legend()
...
class ROCavPhaseDrift_keysight(Measurement1D):
    ...
    def __init__(self, qubit_info, powers, freqs, plot_type=None, qubit_pulse=False, seq=None,  **kwargs):
        ...
        super(ROCavPhaseDrift_keysight, self).__init__(1, infos=(qubit_info,), **kwargs)
