# -*- coding: utf-8 -*-
"""
Created on Tue May  7 17:51:55 2019
...
def analysis(fields, freqs, realdata, imagdata, fig_name, full_fig_name, Sij, fig=None):
    ...
    fields = np.concatenate((fields, np.zeros(1) + fields[1]-fields[0] + fields[-1]))
    ampdata = np.zeros((len(Sij),len(fields), len(freqs)))
    ampdata_a = np.zeros((len(Sij),len(freqs), len(fields)))
    xs,ys = np.meshgrid(fields, freqs/float(1e9))
    gss=[0,0,0,0]
    ...
    #        ampdata = np.zeros((len(realdata),len(fielddata[0,:]), len(freqdata[0,:])))
    #    imag = np.zeros((len(self.fields),len(self.freqs)))
        for i in range(len(fields)-1):
            ampdata[k][i] = 20*np.log10(np.sqrt(realdata[k][i,:]**2 + imagdata[k][i,:]**2))
            
        ampdata[k][len(fields)-1] = 20*np.log10(np.sqrt(realdata[k][i,:]**2 + imagdata[k][i,:]**2))
    #    print Z
    ...
    #    pl.colorbar( a[k] )#, ax = gs[k/2, k%2] )
        fig.axes[k].set_xlabel('fields(T)')
...
class Magnet_Sweep_VNA(Measurement1D):
    ...
    def __init__(self, fields, freqs, average_factor,avelimit,if_bandwidth, Sij, fig_name,comment, **kwargs):
        self.fields = fields
#        print 'self.fields', self.fields
        ...
        self.dfields = fields[1] - fields[0]
#        self.sleeptime_field = sleeptime_field
#        self.plot_type = plot_type
#
        super(Magnet_Sweep_VNA, self).__init__(1, **kwargs)
#        self.set_attrs(
#            title=self.fig_name,
...
#            mainlooptime = 100,
#            stop_at = '0/%s'%(len(self.fields)),
#            avelimit = self.avelimit,   
#        )
        print self.data.get_fullname()
        self.full_fig_name = self.data.get_fullname()
        self.data.create_dataset('fields', data=self.fields)
        self.data.create_dataset('freqs', data=self.freqs)
        
    
#        self.fielddata = self.data.create_dataset('fields', shape=[1,len(self.fields)])
#        self.freqdata = self.data.create_dataset('freqs', shape=[1,len(self.freqs)])
##        self.ampdata = self.data.create_dataset('amplitudes', shape=[len(self.fields),len(freqs)])
##        self.phasedata = self.data.create_dataset('phases', shape=[len(self.fields),len(freqs)])
#        print self.freqdata.get_fullname()
        self.realdata = [0,0,0,0]
        self.imagdata = [0,0,0,0]
        for i, sij in enumerate(self.Sij):
            
            self.realdata[i] = self.data.create_dataset('real%s'%(sij), shape=[len(self.fields),len(freqs)])
            self.imagdata[i]= self.data.create_dataset('imaginary%s'%(sij), shape=[len(self.fields),len(freqs)])
#        
...
#        self.fielddata[0,:] = self.fields
        
        xs,ys = np.meshgrid(self.fields, self.freqs)
...
#                VNA.set_trigger_source('internal')
    ...
    #                    self.freqdata[0,:] = freqs
                #        print freqs
                #        print self.freqdata
                        self.realdata[i][ifield,:] = reals
                        self.imagdata[i][ifield,:] = imags
            
            
                    else:
                        reals =( reals *ave + self.realdata[i][ifield,:] * count)/float(ave+count)
                        imags =( imags *ave + self.imagdata[i][ifield,:] * count)/float(ave+count)
                        self.realdata[i][ifield,:] = reals
                        self.imagdata[i][ifield,:] = imags
        ...
        #            fig.axes[0].title = Sij[0]
            ...
            for i in range(len(self.Sij)):
                z1 = 20*np.log10(self.realdata[i][ifield,:]**2 + self.imagdata[i][ifield,:]**2)
                z1 = z1[:,None].T
                z2 = 20*np.log10(self.realdata[i][ifield,:]**2 + self.imagdata[i][ifield,:]**2)
                z2 = z2[:,None].T
                z = np.concatenate([z1,z2])
                z = np.transpose(z)
                x = np.zeros((len(self.freqs),2))
                x[:,0] = self.fields[ifield]
                x[:,1] = self.fields[ifield]+self.dfields
...
#        print 'self.ampdata\n', self.ampdata
    ...
    def analyze(self):
        analysis(self.fields, self.freqs, self.realdata, self.imagdata, self.fig_name,self.full_fig_name, self.Sij, fig = None)