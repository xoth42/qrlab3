# -*- coding: utf-8 -*-
"""
Created on Mon Jan 18 15:27:08 2021
...
#    print int(title[0:6]) <= 020617
    if int(title[0:6]) <= int('171436') and int(title[0:6]) > int('163952') and title[7:13] == exp_t:# and title[7:12] =='ROCav':
        print(title)
...
#        phase_data = exp['phases']
        if avg_count % 2 == 0:
            avg_array_g[avg_count//2] = data
            
            avg_count += 1
            
            
        elif avg_count % 2 == 1:
            avg_array_e[avg_count//2] = data
...
if final_plot:
    plt.figure()
    plt.title('photon ramsey g/e projection')
    plt.xlabel('frequency')
    plt.ylabel('amplitude')
    for i in range(len(fields)):
        plt.plot(xs,np.abs(avg_data_g+.4)[i], label = 'g %s T'%(fields[i]))
        plt.plot(xs,np.abs(avg_data_e)[i], label = 'e %s T'%(fields[i]))
    plt.legend()
    
#    plt.figure()
#    plt.title('photon ramsey g phase')
#    plt.xlabel('frequency')
#    plt.ylabel('amplitude')
#    for i in range(len(fields)):
#        plt.plot(xs,np.angle(avg_data_g)[i], label = '%s T'%(fields[i]))
#    plt.legend()
    
#    plt.figure()
#    plt.title('photon ramsey e amp')
#    plt.xlabel('frequency')
#    plt.ylabel('amplitude')
#    for i in range(len(fields)):
#        plt.plot(xs,np.abs(avg_data_e)[i], label = '%s T'%(fields[i]))
#    plt.legend()
            
#    plt.figure()
#    plt.title('photon ramsey e phase')
#    plt.xlabel('frequency')
#    plt.ylabel('amplitude')
#    for i in range(len(fields)):
#        plt.plot(xs,np.angle(avg_data_e)[i], label = '%s T'%(fields[i]))
#    plt.legend()
    
    plt.figure()
    plt.title('photon ramsey e-g amp')
    plt.xlabel('frequency')
    plt.ylabel('amplitude')
    for i in range(len(fields)):
        plt.plot(xs,np.abs(avg_data_e-avg_data_g)[i], label = '%s T'%(fields[i]))
    plt.legend()
    
#    plt.figure()
#    plt.title('photon ramsey e-g phase')
#    plt.xlabel('frequency')
#    plt.ylabel('amplitude')
#    for i in range(len(fields)):
#        plt.plot(xs,np.angle(avg_data_e-avg_data_g)[i], label = '%s T'%(fields[i]))
#    plt.legend()
