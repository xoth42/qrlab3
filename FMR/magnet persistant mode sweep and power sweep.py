...
freqs = np.linspace(10.78e9,10.85e9 , 1601)
...
#    average_factor = np.zeros(len(a)) + 7500
    print (np.sum(average_factor)*1.6 + len(average_factor)*10)/3600
#    average_factor = np.ceil(np.power(10, -power/10 )) + 9
#    print (np.sum(average_factor)*1.6 + len(power)*10)/3600
    ro = power_sweep_VNA.Power_Sweep_VNA(powers = power, freqs = np.linspace(10.78e9,10.85e9,1601),
                                                   average_factor = average_factor, avelimit = 40,if_bandwidth =1000, Sij =['S21'],fig_name ='S31 power sweep at %sT '%(field),comment = '',sweep_SC_qubit = False)
    ro.measure()
