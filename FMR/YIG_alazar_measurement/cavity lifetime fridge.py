...
#    m = magnet.do_get_field()
    ...
    filename = '04032018overnight_1.5mm_%sdB_%sT'%(p,m)
...
#        Yoko.do_set_voltage(0)    
        plt.figure()
        plt.plot(freq_range, Amp,label=' magnetic field = %s T \n Amp'%( m))
    ...
    #            plt.scatter(array(mintab)[0][0], array(mintab)[0][1], color='red')
        ...
        np.savetxt(r'alazar_sweep_fridge_%s_%sT_%s GHz-%s GHz_%s.txt'%(filename,m,start_freq/float(1e9),stop_freq/float(1e9),num) ,data1, delimiter=",") # saves data
...
#==============================================================================
    if 1:    # fitting the modulated data
        start_fitting = 2180
        sleeptime =1
        n=30000# number of average traces
#        Yoko.do_set_voltage(mag/float(40))       
...
#                time.sleep(0.002)
    ...
    #            print i
            ...
            np.savetxt(r'decay_graph_%s_%s GHz_%s.txt'%(filename,freq/float(1e9),n) ,data_decay1, delimiter=",") # saves data   
