...
def morning_check():
    ...
    if 1: #Rabi checking pi amps for lower qubit from both input lines
...
#                          sequencer.Combined([cx_info.rotate(np.pi, 0), cancel_info.rotate(np.pi, 0)])
#    postseq =  gate_info1.rotate(np.pi,0)
    tr1 = rabi.Rabi(cx_info, np.linspace(0.070,0.083, 101), selective=False,
                                   plot_seqs=False, generate=True, repeat_pulse=8, cancel_info=cancel_info,
...
if 0: # Tune up for time vs relative amp
    ...
    cr_tune = CRtuning_time_amp.CRtuning_time_amp(cx_info, gate_info2,  
                                                    np.linspace(1,600,31), rel_amps=np.linspace(4.4,4.5,11), 
                amp=0.0765, phase=0, rel_phase=0.983, sigma=6, seq=seq_cool, postseq=Y_proj, cancel_info=cancel_info,
                control_pi=False, proj_func='phase', extra_info=gate_info1)    
    data = cr_tune.measure()
    bla
    
if 1: # Tune up for time vs relative phase
    alz.set_naverages(3500)
    rotation = 0.0
    X_proj = gate_info1.rotate(np.pi/2, np.pi/2+rotation)
    Y_proj = gate_info1.rotate(np.pi/2, rotation)

    cr_tune = CRtuning_time_phase.CRtuning_time_phase(cx_info, gate_info2,  
                                                    np.linspace(1,400,21), rel_phases=np.linspace(0.96,1.04,21), 
                amp=0.0765, phase=0, rel_amp=4.457, sigma=6, seq=seq_cool, postseq=None, cancel_info=cancel_info,
                control_pi=True, proj_func='phase',plot_seqs=False, extra_info=gate_info1)    
    data = cr_tune.measure()
    bla
...
if 0: # Tune up for time vs detuning   
    ...
    cool = sequencer.Constant(int(4e3),1,chan='3m1')
    seq_cool = sequencer.Join([sequencer.Trigger(250), cool, sequencer.Delay(150)])     
    cr_tune = CRtuning_timevsdet.CRtuning_timevsdet(qubit2_info2, qubit2_info, gate_info1, 
...
#    rel_phase = 1.77#-np.pi
    for postseq in [None]:
        tr = timerabi_interleaved.TimeRabi_interleaved(
            qubit2_info2, qubit_info2, gate_info1, np.linspace(0, 100, 41), #Does not include Gaussian ramp time, sigma=4
            amp=0.3, phase=0, rel_amp=rel_amp, rel_phase=rel_phase, sigma=6,
...
#    rotation = -0.08
    ...
    X_proj = gate_info1.rotate(np.pi/2, np.pi/2+rotation)
    Y_proj = gate_info1.rotate(np.pi/2, rotation)
    for rel_amp in [4.433]:
#    rel_amp= 4.457#5.03#4.4493
        for rel_phase in [0.993]: #1.011
#    rel_amp=4.4405
#    rel_phase=1.01
            
            for postseq in [X_proj]:
                tr = timerabi_interleaved.TimeRabi_interleaved(
                            cx_info, gate_info2, np.linspace(0, 600, 101), #Does not include Gaussian ramp time, sigma=4
                            amp=0.0766, phase=0, rel_amp=rel_amp, rel_phase=rel_phase, sigma=6, read_on_e=True, cancel_info=cancel_info, 
                            update=False, seq=seq_cool, postseq=postseq, proj_func='phase', plot_seqs=False, extra_info=gate_info1)
                data = tr.measure()
...
if 0: # single qubit tune up for time vs relative amp for  #NEED TO FIX THE 2D PLOTTING ISSUE
    ...
    sq_tune = Singlequbit_tuning_time_amp.Singlequbit_tuning_time_amp(qubit_info, qubit_info2, gate_info2, 
                                                    np.linspace(1,400,21), rel_amps=np.linspace(5.0,5.2,9), 
                amp=0.065, phase=0, rel_phase=0.980, sigma=4, seq=seq_cool, proj_func='phase')    
