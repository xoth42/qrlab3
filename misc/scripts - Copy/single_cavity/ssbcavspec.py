...
class SSBCavSpec(Measurement1D):
    ...
    def __init__(self, qubit_info, cav_info, detunings, seq=None, extra_info=None, saveas=None, **kwargs):
        ...
        super(SSBCavSpec, self).__init__(len(detunings), residuals=False, **kwargs)
    ...
    def generate(self):
        ...
        for i, df in enumerate(self.detunings):
            g = DetunedGaussians(self.cav_info.sigma, chans=self.cav_info.sideband_channels)
            if df != 0:
                period = 1e9 / df
            else:
                period = 1e50
            g.add_gaussian(0, period, area=self.cav_info.rotate.pi_area)
