...
class Sequencer:
    ...
    def __init__(self, s=None, minlen=MINLEN, ch_align=True):
        self.pulse_data = {}
        self.chan_props = {}
        self.minlen = minlen
        self._ssb_list = []
    ...
    def render_subseq(self, ss, chs, debug=False):
        ...
        if not self.sequences_aligned(seqs):
...
class Instruction(object):
    ...
    def __init__(self, address=None, label=None):
...
class Program:
    ...
    It assigns an address to each instruction and resolves jumps etc.
    '''
...
#            data = np.array([0,])
    ...
    def generate(self, now, chan):
        ...
        if chan == self.chan or self.chan == CHAN_ANY:
            ...
            d = Delay(plen, unroll=True, trigger=self.trigger)
...
class Combined(Instruction):
    ...
    def __init__(self, items, align=ALIGN_LEFT, ch_align=False, ch_delays={}):
...
class SSB:
    ...
    When calibrating dphi using a cosine on I and a sine + dphi on Q and
    optimizing for the LOWER sideband, the dphi to use is pi - dphi and the
