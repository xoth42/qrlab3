...
hdf5_name = '0626cooldown_circualtor_VNA.hdf5'
...
#    imag = np.concatenate((exp['imaginaryS21'].value[::-1], exp_['imaginaryS21'].value),axis = 0)
    ...
    if subtract_final:
        for i in range(len(real)):
            real[i] = real[i] - real[-1]
            imag[i] = imag[i] - imag[-1]
...
#    pl.xlim(X.min(), X.max())
    pl.pcolormesh(X,Y/1e9,Z,vmin = -10)
    pl.colorbar()
#    
...
#    pl.ylabel('Spectroscopy Frequency(GHz)',fontsize = 12)
    ...
    fig.add_subplot(gs[0])
    fig.add_subplot(gs[1])
    if fit_S12 or fit_S11 or fit_S12_two_modes_V2 or fit_S12_two_modes_V3:
        ...
        kappa_a2 = np.zeros(len(real))
        kappa_a2_err = np.zeros(len(real))
        omega_c2 = np.zeros(len(real))
        omega_c2_err = np.zeros(len(real))
        kappa_prod2 = np.zeros(len(real))
        kappa_prod2_err = np.zeros(len(real))
...
#        gs = gridspec.GridSpec(1, 2)
    ...
    #    pl.suptitle(figname)
        ...
        if fit_S12_two_modes_V3:
            params = lmfit.Parameters()
            params.add('kappa_prod1', value= 6e9, min = 0)#,vary = False)
            params.add('omega_c', value=10.929e9)#,vary = False)
            params.add('kappa_a', value=5e6, min = 0)#,vary = False)
            if np.max(np.abs(datas)) < limit_for_off:
                params.add('roff',value =(datas[0].real+ datas[-1].real)/2,vary = False)
                params.add('ioff',value = (datas[0].imag+ datas[-1].imag)/2, vary = False)
            params.add('phi1',value = 1.4, max = 1.5*np.pi, min = -1.5*np.pi)#,vary = False)
                    
        
            params.add('kappa_prod2', value= 3e10, min = 0)#,vary = False)
            params.add('omega_c2', value=10.935e9)#,vary = False)
            params.add('kappa_a2', value=2e6, min = 0)#,vary = False)
            params.add('phi21',value = -1.6, max = 1.5*np.pi, min = -1.5*np.pi)#,vary = False)
        ...
        if fit_S12 or fit_S11 or fit_S12_two_modes_V2 or fit_S12_two_modes_V3:
            ...
            if fit_S12_two_modes_V2:
                kappa_a2[i] = result.params['kappa_fmr'].value
                kappa_a2_err[i] = result.params['kappa_fmr'].stderr
                omega_c2[i] = result.params['omega_fmr'].value
                omega_c2_err[i] = result.params['omega_fmr'].stderr
                fig.axes[0].plot(freqs/float(1e9), fitdatadB1,'--' )
                fig.axes[0].plot(freqs/float(1e9), fitdatadB2,'--' )
                 
            if fit_S12_two_modes_V3:
                kappa_prod[i] = result.params['kappa_prod1'].value
                kappa_prod_err[i] = result.params['kappa_prod1'].stderr
                kappa_prod2[i] = result.params['kappa_prod2'].value
                kappa_prod2_err[i] = result.params['kappa_prod2'].stderr
                kappa_a2[i] = result.params['kappa_a2'].value
                kappa_a2_err[i] = result.params['kappa_a2'].stderr
                omega_c2[i] = result.params['omega_c2'].value
                omega_c2_err[i] = result.params['omega_c2'].stderr
    #    pl.xlabel('freq(GHz)')
...
LW = np.transpose(LW)
powers = np.linspace(-30,10,5)
pl.figure()

for i in range(5):
    pl.errorbar(T,LW[i]/1000000,label = '%sdB'%(powers[i]))
