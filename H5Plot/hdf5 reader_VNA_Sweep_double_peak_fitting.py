...
for i in range(len(real))[11:13]:
    print powers[i]
    fig.axes[1].plot(real[i],imag[i], label= 'power = %sdB'%(powers[i]))
    fig.axes[0].plot(freq/1e9,mag[i], label= 'power = %sdB'%(powers[i]))
    omega_fmr = 0.99*28.025*(powers[i]*1000+178*1.2*(0.444444-0.333333))/1000+0.495 - 1*(powers[i]-0.2678)
    omega_ci = 8.599e9 + 1*(powers[i]-0.2678)
    omega_fmr = omega_fmr *1e9
    ...
    if fit_S12:
        ...
        params.add('kappa_prod', value= 1.8e10, min = 0)#,vary = False)
        params.add('omega_c', value=omega_ci)#,vary = False)
        params.add('kappa_a', value=1.56e6, min = 0)#, max = 4e6)#,vary = False)
        ...
        params.add('g', value= 3.16e6, min = 0)#,vary = False)
    ...
    #    params.add('g', value=2e7, min = 0)
        ...
        fitdata = np.sqrt(result.params['kappa_prod'].value)/(-1j*(freqs-result.params['omega_c'].value)-(result.params['kappa_a'].value)/2.0 + result.params['g'].value**2/(-1j *(freqs-result.params['omega_fmr'].value)-result.params['kappa_fmr'].value/2) )
